...
// ── Store SKU Editor ──────────────────────────────────────────────────────────
// Operator surface for editing the store_skus catalog: which asset categories
// each SKU grants access to. Backed by /api/admin/store-skus.
//
...
function StoreSkusPanel() {
  ...
  const toggleFlag = useCallback(async (skuId: string, key: keyof StoreSkuRow, current: boolean) => {
    setSavingSku(skuId);
    setError(null);
    // Optimistic local update — revert if the API rejects.
