 ...
 * Content comes from /api/codex/knyt/terra (nakamoto_content_items filtered
 * to knyt/metaknyt tags or qriptopian strand).
 *
 * Engagement signals (share, like, spark) are routed to
 * POST /api/codex/qriptopian/signal, which:
 *   - Logs to Qc event ledger (cartridgeId='qriptopian')
 *   - Emits a DVN participation receipt
 *   - Grants HeraldCuriosityClicks reward for shares with a personaId
 *   - Inserts into knyt_signals for like/spark
 */
...
async function emitSignal(
  ...
  try {
    ...
    return json.data?.reward ?? { granted: false };
  } catch {
  ...
  const handleShare = useCallback(async () => {
    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        ...
        // User cancelled or API not supported — still log the intent
      }
    ...
    // Emit signal and check for reward
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {/* Reward toast */}
            {rewardEarned !== null && (
              <span className="absolute -top-7 right-0 text-[10px] font-bold text-amber-300 bg-amber-900/80 border border-amber-500/30 rounded-full px-2 py-0.5 whitespace-nowrap animate-fade-in">
                +{rewardEarned} $KNYT
              </span>
            )}
          </button>
...
export function TerraTab({ personaId }: TerraTabProps) {
  ...
  useEffect(() => {
    fetch("/api/codex/knyt/terra?limit=20")
      .then((r) => r.json())
      .then((json) => {
        if (json.ok) {
          setItems(json.data ?? []);
