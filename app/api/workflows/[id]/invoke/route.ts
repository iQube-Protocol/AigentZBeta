...
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // Update health state based on invocation outcome
    await updateBinding(binding.id, {
      healthState: result.ok ? "healthy" : "degraded",
      lastHealthCheckedAt: new Date().toISOString(),
    }).catch(() => {}); // non-blocking

    return NextResponse.json({ ok: result.ok, executionId: result.executionId, output: result.output, error: result.error });
  } catch (err: any) {
