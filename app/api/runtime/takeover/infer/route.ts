...
async function fetchContentCatalog(config: RuntimeTakeoverConfig): Promise<CatalogEntry[]> {
  ...
  // cartridge that already accepts smart-content (KNYT) gets these items in
  // its takeover catalog. The runtime client matches by id against the
  // capsule record list; promoted rows are projected into RuntimeCapsules
  // by /api/runtime/capsules using the same `community-<id>` key.
  if (
