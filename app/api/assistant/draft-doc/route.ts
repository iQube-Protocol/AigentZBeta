/**
 * POST /api/assistant/draft-doc
 *
 * Aigent Me Phase 6.b Part 2.5c — Google Doc drafter.
...
import { draftGoogleDoc, type DraftGoogleDocContext } from '@/services/agents/draftGoogleDoc';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!prompt) {
    return NextResponse.json({ error: 'missing-prompt' }, { status: 400, headers: { 'Cache-Control': 'no-store' } });
  }
  const ctx: DraftGoogleDocContext = {};
  try {
  ...
  const draft = await draftGoogleDoc({ prompt, context: ctx });
