/**
 * POST /api/assistant/draft-sheet
 *
 * aigentMe Google Sheets drafter.
...
import { draftGoogleSheet, type DraftGoogleSheetContext } from '@/services/agents/draftGoogleSheet';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  const ctx: DraftGoogleSheetContext = {};
  try {
  ...
  const draft = await draftGoogleSheet({ prompt, context: ctx });
