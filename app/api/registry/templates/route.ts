  ...
  try {
    ...
    if (supabase) {
      ...
      let query = supabase.from('iqube_templates').select(
        'id,name,description,iqube_type,instance_type,business_model,price,version,provenance,parent_template_id,blakqube_labels,sensitivity_score,accuracy_score,verifiability_score,risk_score,created_at'
      );
      const search = params.get('search');
      if (search) {
