...
async function postImpl(request: NextRequest) {
  ...
  // aigentMe activation is idempotent — only write it once per persona PER
  // AGENT. Scoped by agentsInvoked so a prior activation for a different
  // agent (e.g. MoneyPenny) never shadows a genuinely fresh one for this one.
  ...
  if (existing.length === 0) {
    const failure = await record('aigentme_activated', () =>
      createActivityReceipt({
        ...
        summary: `aigentMe activated as the principal's constitutional companion, regarding ${agent.displayName}`,
        agentsInvoked: [agent.runtimeAgentId],
      }),
    );
  ...
  const failure = await record('experienceqube_focus_disposition_recorded', async () => {
    receipt = await createActivityReceipt({
      ...
      summary: `Principal recorded disposition '${disposition}' on ${agent.displayName}'s Financial Services domain focus`,
      agentsInvoked: [agent.runtimeAgentId],
