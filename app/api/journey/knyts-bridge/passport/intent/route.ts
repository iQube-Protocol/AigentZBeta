/**
 * POST /api/journey/knyts-bridge/passport/intent
 *
 * PASSPORT's post-crossing "What would you like to do in the Polity?"
 * signal (2026-08-12, KNYTS↔CI parity pass) — the KNYTS-scoped twin of
 * .../constitutional-internet-bridge/passport/intent/route.ts, both fed by
 * the SAME shared `BridgeActionModeQuestion` component. A preference/signal
 * for aigentMe, NOT an authority grant. It is not constitutional state,
 * never gates any JourneyDefinition stage, and is never presented as
 * Standing or delegation. Best-effort via the existing generic campaign
 * event log (services/campaign/campaignService.ts's recordCampaignEvent
 * writes to the already-existing `campaign_events` table — no new schema),
