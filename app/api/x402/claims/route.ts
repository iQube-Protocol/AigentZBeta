...
export async function GET(req: NextRequest) {
  ...
  if (r1.data && r1.data.length > 0) return NextResponse.json({ ok: true, data: r1.data }, { headers: corsHeaders });
  const r2 = await run(didNoFrag);
  if (r2.data && r2.data.length > 0) return NextResponse.json({ ok: true, data: r2.data }, { headers: corsHeaders });
  ...
  const r3 = await q3;
  if (r3.data && r3.data.length > 0) return NextResponse.json({ ok: true, data: r3.data }, { headers: corsHeaders });
  ...
  const r4 = await q4;
  if (r4.data && r4.data.length > 0) return NextResponse.json({ ok: true, data: r4.data }, { headers: corsHeaders });
  if (r1.error) return NextResponse.json({ ok: false, error: r1.error.message }, { status: 500, headers: corsHeaders });
  if (r2.error) return NextResponse.json({ ok: false, error: r2.error.message }, { status: 500, headers: corsHeaders });
  if (r3.error) return NextResponse.json({ ok: false, error: r3.error.message }, { status: 500, headers: corsHeaders });
