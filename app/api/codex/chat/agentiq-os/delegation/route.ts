 ...
 *
 * Manages HandoffPayload + PolicyEnvelope per the bounded delegation model.
 * Phase 1: in-memory store (server restart clears). Phase 2: Supabase persistence.
 *
 * All lifecycle events are logged with receipt-eligible metadata.
 * DVN receipt anchor: did:iqube:aigent-c-os-root (agent Root DiD).
 */

import { NextRequest, NextResponse } from 'next/server';
import type { PolicyEnvelope, HandoffPayload } from '@/types/orchestration';
...
// Phase 1: in-memory store keyed by persona_id
// Phase 2: replace with Supabase orchestration_events / delegation_grants table
...
// Allowed actions per trust band
...
// Base forbidden actions — always blocked regardless of trust band
...
function logEvent(
  ...
  // Phase 1: console log with receipt-eligible metadata
  // Phase 2: write to Supabase orchestration_events table
  console.info('[Delegation Event]', {
...
export async function POST(request: NextRequest) {
  try {
    ...
    // L5 requires guardian approval (not implemented in Phase 1)
    ...
    logEvent('z_delegated', persona_id, {
...
// ============================================================================
// GET — Read active delegation state
// ============================================================================
...
export async function GET(request: NextRequest) {
  ...
  if (isExpired(record)) {
    ...
    logEvent('control_returned_to_metame', persona_id, {
