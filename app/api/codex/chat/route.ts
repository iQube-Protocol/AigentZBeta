...
const LAYOUT_KEYWORDS: Array<{ id: ChipTargetId; pattern: RegExp; reason: string }> = [
  ...
  { id: 'gmail',            pattern: /(draft (an? )?email|gmail|send (an? )?email|outreach email|email draft|reply to|send (?:it|the (?:doc|deck|brief|file|presentation|proposal|report|link))\s+to\b)/i, reason: 'Operator wants to draft an email' },
  ...
  { id: 'slides',           pattern: /(slide deck|presentation|create (a )?deck|pitch deck|slides outline|google slides|(?:proposal|partner|launch|investor|go-to-market)\s+deck)/i, reason: 'Operator wants to create a slide deck' },
