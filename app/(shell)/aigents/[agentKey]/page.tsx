...
export default function AgentPage({ params }: { params: { agentKey: string } }) {
  ...
  const [systemChatOpen, setSystemChatOpen] = React.useState(false);
  const isAigentZ = params.agentKey === "aigent-z";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {isAigentZ && (
            <button
              onClick={() => setSystemChatOpen(true)}
              className="px-3 py-1.5 text-xs rounded border border-emerald-700 bg-slate-800 text-emerald-300 hover:bg-slate-700"
            >
              System AI
            </button>
          )}
      ...
      {isAigentZ && (
        <AigentZSystemChat open={systemChatOpen} onClose={() => setSystemChatOpen(false)} />
      )}
    </div>
  );
}
