...
import { X, User, Wallet, Bitcoin, Save, Loader2, CheckCircle, AlertCircle, Camera } from 'lucide-react';
  ...
  useEffect(() => {
    if (isOpen && persona.id) {
      ...
      const fetchPersonaData = async () => {
        ...
        try {
          const response = await fetch(`/api/identity/persona/${persona.id}`);
          if (response.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.referrerValid && form.referrerId && !form.referralLockedAt) {
        await fetch(`/api/referrals/set`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            personaId: form.id,
            referrerId: form.referrerId,
            referralIdentifier: form.referrerIdentifier
          })
        });
