...
// ─── 10. Operator-assisted artifact registration (admin path only) ──────────

/**
 * Register Party B's artifact on behalf of a principal who holds an active
 * research-lab grant and verified Passport, WITH the principal's own later
 * attestation gated before freeze/sign can proceed. Server-side only; the
 * admin route verifies caller auth, computes SHA-256 from received bytes,
 * verifies Passport + grant eligibility, and calls this function with exact
 * provenance.
 *
 * Idempotent: same (exchangeId, partySlot, artifactHash) → same artifact,
 * not a duplicate.
 */
export async function registerArtifactOperatorAssisted(
  admin: SupabaseClient,
...
): Promise<{ ok: true; artifact: ExchangeArtifactRecord } | { ok: false; reason: string; error?: string }> {
  const loaded = await loadExchange(admin, input.exchangeId);
  if (!loaded.ok) return { ok: false, reason: 'exchange-not-found', error: loaded.error };
  const { exchange } = loaded;

  // Verify the target party slot is unoccupied or already has this exact artifact.
  const existing = await currentArtifact(admin, input.exchangeId, input.partySlot);
  if (existing) {
    if (existing.contentHash === input.artifactHash) {
      return { ok: true, artifact: existing };
    }
    return { ok: false, reason: 'party-already-has-artifact', error: `Party ${input.partySlot} already has an artifact with a different hash — cannot overwrite without explicit replacement workflow` };
  }

  // Party A (initiator) is immutable. Operator-assisted registration only applies to Party B.
  if (input.partySlot === 'A') {
    return { ok: false, reason: 'party-a-immutable', error: 'Party A artifact registration must go through the normal deposit flow, not operator-assisted' };
  }

  // Insert the operator-assisted artifact row with pending flag.
  const { data, error } = await admin
    .from(T_ARTIFACTS)
    .insert({
      exchange_id: input.exchangeId,
      party: input.partySlot,
      title: `[Operator-registered] Boundary Research artifact`,
      artifact_class: 'operator-registered-deposit',
      version: 1,
      source_type: 'immutable-reference',
      source_reference: 'operator-assisted-registration',
      content_hash: input.artifactHash,
      mime_type: input.mimeType,
      ownership_declaration: 'Registered via operator-assisted workflow under boundary research exchange admission protocol.',
      rights_for_exchange: 'Authorized under active research-lab grant and verified Passport.',
      registering_operator_persona_id: input.registeringOperatorPersonaId,
  ...
  if (error || !data) {
    return {
      ...
      error: isMissingTable(error) ? MIGRATION_HINT : error?.message ?? 'artifact registration failed',
    };
  }

  const artifact = rowToArtifact(data as Record<string, unknown>);

  // Emit activity receipt with all three identities.
  const receipt = await createActivityReceipt({
    personaId: input.boundPrincipalId,
    activeCartridge: 'irl',
    actionType: 'exchange_artifact_registered_operator_assisted',
    summary: `Artifact registered by operator [exchange=${input.exchangeId}] party=${input.partySlot} fingerprint=${artifact.contentHash?.slice(0, 16)} — awaiting principal attestation`,
    contextShared: ['exchange_id', 'content_hash'],
  }).catch(() => null);
...
/**
 * Principal attestation: clear the pending flag after the principal confirms
 * they are taking ownership of the operator-registered artifact. Callable
 * ONLY by the principal bound to the artifact (enforced server-side).
 * Idempotent: re-calling with the same artifact already in confirmed state
 * returns success.
 */
export async function confirmOperatorAssistedArtifact(
  admin: SupabaseClient,
  input: {
    exchangeId: string;
    artifactId: string;
    confirmingPersonaId: string;
    sourceChannel: EvidenceOriginChannel;
  },
): Promise<
  | { ok: true; artifact: ExchangeArtifactRecord }
  | { ok: false; reason: string; error?: string }
> {
  // Load exchange and artifact.
  const loaded = await loadExchange(admin, input.exchangeId);
  if (!loaded.ok) return { ok: false, reason: 'exchange-not-found', error: loaded.error };
  const { exchange } = loaded;
  ...
  if (artifactError) {
    return {
      ...
      error: isMissingTable(artifactError) ? MIGRATION_HINT : artifactError.message,
    };
  }

  if (!artifactRow) {
    return { ok: false, reason: 'artifact-not-found', error: `No artifact ${input.artifactId} in exchange ${input.exchangeId}` };
  }

  const artifact = rowToArtifact(artifactRow as Record<string, unknown>);

  // Verify the confirming persona matches the bound principal.
  if (artifact.boundPrincipalId !== input.confirmingPersonaId) {
    return {
      ok: false,
      reason: 'wrong-principal',
      error: `This artifact is bound to ${artifact.boundPrincipalId}; you are ${input.confirmingPersonaId}. Only the bound principal can confirm attestation.`,
    };
  }
  ...
  if (updateError || !updated) {
    return { ok: false, reason: 'update-failed', error: updateError?.message ?? 'failed to clear pending flag' };
  }

  const confirmedArtifact = rowToArtifact(updated as Record<string, unknown>);

  // Emit activity receipt.
  const receipt = await createActivityReceipt({
    personaId: input.confirmingPersonaId,
    activeCartridge: 'irl',
    actionType: 'exchange_operator_assisted_artifact_confirmed',
    summary: `Principal confirmed attestation [exchange=${input.exchangeId}, artifact=${input.artifactId}] — pending principal attestation cleared`,
    contextShared: ['exchange_id', 'artifact_id'],
  }).catch(() => null);

  return { ok: true, artifact: confirmedArtifact };
}
