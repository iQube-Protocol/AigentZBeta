...
export async function requestPersonaSwitch(session: ScopedSession, input: { personaPublicRef: string; codeChallenge: string; state: string }, origin: string): Promise<{ ok: true; authorizeUrl: string; expiresAt: string } | { ok: false; error: string }> {
  const targetRef = input.personaPublicRef.trim(); const oauthState = input.state;
  ...
  const prepared = await createPersonaSwitchHandshake({ source: session, targetPrincipalPublicRef: targetRef, requestedScope: rootScopeForTarget(target.cartridgeFlags.isAdmin), pkceChallenge: input.codeChallenge, oauthState });
  if ('error' in prepared) return { ok: false, error: prepared.error };
  return { ok: true, authorizeUrl: `${origin}/threshold/switch-persona#code=${encodeURIComponent(prepared.handshakeCode)}`, expiresAt: prepared.expiresAt };
}
