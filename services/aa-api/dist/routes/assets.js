...
assetsRouter.post('/upload/initiate', requireAuth, async (req, res) => {
    ...
    if (!media_kind || !bytes || !owner_did)
        return res.status(400).json({ error: 'media_kind, bytes, owner_did required' });
...
assetsRouter.post('/register', requireAuth, async (req, res) => {
    ...
    if (!asset_id || !sha256)
        return res.status(400).json({ error: 'asset_id & sha256 required' });
...
assetsRouter.post('/:id/policy', requireAuth, async (req, res) => {
    ...
    if (!pay_to_did)
        return res.status(400).json({ error: 'pay_to_did required' });
