...
paymentsRouter.post('/quote', requireAuth, async (req, res) => {
    try {
        const { did: buyer_did, tenant_id } = req.auth;
        ...
        if (!policy)
            return res.status(400).json({ error: 'no policy for asset' });
        const payload = {
        ...
        if (process.env.X402_AUTOSUBMIT === 'true') {
            try {
                const submitResp = await submitToFacilitator(headers, body);
                await supabase
                    .from('x402_transactions')
...
paymentsRouter.post('/notify', async (req, res) => {
    try {
        ...
        if (!request_id || !status)
            return res.status(400).json({ error: 'request_id and status required' });
        // Load transaction
        const { data: tx, error: terr } = await supabase
            .from('x402_transactions')
            .select('*')
...
paymentsRouter.post('/submit', requireAuth, async (req, res) => {
    try {
        ...
        const headers = { 'x-x402-signature': signature };
        const body = payload;
        const resp = await submitToFacilitator(headers, body);
        const { data: updated, error: uerr } = await supabase
            .from('x402_transactions')
