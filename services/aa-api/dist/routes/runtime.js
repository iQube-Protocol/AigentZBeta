...
function parseAuthPayload(authorizationHeader) {
    const header = authorizationHeader || '';
    const token = header.startsWith('Bearer ') ? header.slice(7) : null;
    if (!token)
        return null;
    try {
        return jwt.verify(token, env.AA_JWT_SECRET);
    }
    catch {
        return null;
    }
}
function normalizeDeviceHint(value) {
    if (typeof value !== 'string')
        return null;
...
function resolveRuntimeContext(req) {
    const authPayload = parseAuthPayload(req.headers?.authorization);
    const body = (req.body || {});
    const query = (req.query || {});
    const payload = (body.payload || {});
    const tenantId = pickString(query.tenant_id, query.tenantId, body.tenant_id, body.tenantId, req.headers?.['x-tenant-id'], authPayload?.tenant_id, authPayload?.tenantId, env.DEFAULT_TENANT_ID, 'metame') || 'metame';
    const personaId = pickString(query.persona_id, query.personaId, body.persona_id, body.personaId, req.headers?.['x-persona-id'], authPayload?.persona_id, authPayload?.personaId, authPayload?.did, env.DEFAULT_PERSONA_ID, 'guest') || 'guest';
    const deviceHint = normalizeDeviceHint(query.device) ||
        ...
        'mobile';
    const processing = pickBoolean(query.processing, query.is_processing, body.processing, body.is_processing, payload.processing, payload.is_processing, req.headers?.['x-runtime-processing']) ?? false;
    return {
        tenantId,
        personaId,
...
function providerReliability(providerId, processing = false) {
    let base = 4.5;
...
function trustStateFromScore(score) {
    if (score >= 8)
        return 'ok';
    if (score >= 5)
        return 'warn';
...
function getLlmOptionsForAgent(agentId) {
    return LLM_OPTIONS_BY_AGENT[agentId] || LLM_OPTIONS_BY_AGENT['aigent-z'] || [];
}
function getLlmById(agentId, llmId) {
...
function normalizeMode(value) {
    if (value === 'triad' || value === 'collapsed' || value === 'full')
        return value;
    return null;
}
...
function normalizeRuntimeIframePath(pathname) {
...
function normalizeRuntimeIframeUrl(input) {
    try {
...
function resolveIframeUrl() {
    const raw = asString(env.RUNTIME_IFRAME_URL) ||
...
function resolveIframeOrigin(iframeUrl) {
    const iframeUrlOrigin = parseOrigin(iframeUrl) || 'http://localhost:3000';
    const explicit = asString(env.RUNTIME_IFRAME_ORIGIN) ||
...
function buildShellConfig(ctx, state) {
    ...
    return {
        ...
        session: {
            ...
            trust_signals: [
                { key: 'did', label: 'DID Verified', state: ctx.authMode === 'jwt' ? 'ok' : 'warn' },
                { key: 'iqube', label: 'iQube Secured', state: 'ok' },
...
function updateStateForAgent(state, aigentId, llmId) {
...
function updateStateForModel(state, llmId) {
...
function inferIntentFromPrompt(prompt, fallback = 'find') {
    const lower = prompt.toLowerCase();
    if (lower.includes('be') || lower.includes('identity') || lower.includes('persona'))
        return 'be';
    if (lower.includes('earn') || lower.includes('reward') || lower.includes('payout'))
        return 'earn';
    if (lower.includes('play') || lower.includes('watch') || lower.includes('listen') || lower.includes('read'))
        return 'play';
    if (lower.includes('make') || lower.includes('create') || lower.includes('build'))
        return 'make';
    if (lower.includes('share') || lower.includes('find') || lower.includes('discover'))
        return 'find';
    return fallback;
}
runtimeRouter.get('/shell-config', (req, res) => {
    const ctx = resolveRuntimeContext(req);
    const modeOverride = normalizeMode(req.query?.mode);
...
runtimeRouter.post('/prompt-action', (req, res) => {
    ...
    const requestedMode = normalizeMode(body.payload?.menu_mode);
    const next = {
        ...current,
        welcome_complete: true,
    ...
    return res.json({
        ...
        prompt_event: {
            ...
            surface_plan_instruction: matchingAction?.trigger.surface_plan_instruction || 'route prompt through runtime intent planner',
            copilot_instruction: matchingAction?.trigger.copilot_instruction || 'route prompt to metaMe copilot and sync resulting state',
            iframe_event: {
                type: 'PROMPT_SUBMIT',
                payload: {
                    text: prompt,
