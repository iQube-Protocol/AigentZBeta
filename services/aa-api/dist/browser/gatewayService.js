...
function buildMockDataUrlHtml(session) {
...
export class BrowserGatewayService {
    liveViewUrls = new Map();
    connectUrls = new Map();
    rememberProviderState(session, input) {
    ...
    async createProviderSession(input) {
    ...
    async navigate(session, url, action) {
        ...
        const connectUrl = this.connectUrls.get(session.sessionId) ||
    ...
    async closeProviderSession(session) {
        if (session.provider !== 'browserbase' || !browserbaseProviderAdapter.isConfigured()) {
    ...
    async updateMountPayload(session, surfaceState, previous) {
        let liveViewUrl = this.liveViewUrls.get(session.sessionId) ||
