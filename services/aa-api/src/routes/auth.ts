...
const challenges = new Map<string, string>(); // did -> nonce
...
authRouter.post('/verify', async (req, res) => {
  ...
  if (!did || !signature) return res.status(400).json({ error: 'did & signature required' });
  const nonce = challenges.get(did);
  if (!nonce) return res.status(400).json({ error: 'no active challenge' });
