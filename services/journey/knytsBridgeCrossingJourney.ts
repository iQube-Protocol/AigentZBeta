...
export const KNYTS_BRIDGE_CROSSING_JOURNEY: JourneyDefinition = {
  ...
  stages: [
    ...
    // ── Financial Sovereignty segment (AEE-XP-001 §4.2, Phase 1 main-spine
    //    connection) — the KNYTS on-ramp into the Financial Services Bridge.
    //    Reachable from Choose's "Apply to join the Constitutional Financial
    //    Services Pilot" card. Gate-less like HOME/VIEW/ORIENT/CHOOSE (an
    //    informational + handoff segment, not a tracked constitutional
    //    admission ladder) — see this file's own "seven spine nodes, three
    //    tracked stages" header for why a gate-less node is still a real
    //    spine node rather than a page section adrift from the spine.
    {
      id: 'fs-discover',
      label: 'Discover',
