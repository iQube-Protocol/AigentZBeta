...
Not a **fact**, not **knowledge**, not **truth** — *previously validated reasoning*. This is the criterion that also grounds IRL Principle 005 (`inv.epistemology.133`). This definition is **technically correct and fully consistent with §0.7** (the invariant *is* a real regularity): the clarification is that a structural invariant is **accessed and codified by an intelligent system only as a representation**. Gravity is real, but a system reaches it *through representations of it* — so the invariant *as held by a system* is a persistent representation of the validated reasoning about a regularity that is itself real. Two things, not three: the **real invariant** (§0.7) and its **representation** — which is simultaneously the codified held form (this §0.4) and the interface (§0.5/§0.8). *(Canonical: `inv.epistemology.139`; reconciliation `inv.epistemology.150`.)*
...
> **Structural invariants are anchored in reality; constitutional invariants are properties of the systems that reason about reality.** The first is the science of Invariant Intelligence; the second is the engineering of Constitutional Computing; where they meet is the operationalization of truth.

**Structural invariants are part of reality, not proxies for it.** Take gravity: it is an invariant of nature *irrespective of any representation* — planetary mass and distance modulate it, but the invariant lets us abstract the core regularity as a substrate of the whole system. Humans experience gravity directly; we *reason about* it through physics and mathematics — **the representation is the proxy interface (§0.8) through which an intelligent system, natural or synthetic, reasons on a structural invariant that is more fundamentally anchored in reality.** The invariant is the reality; the representation is how a system reaches it. This resolves the earlier hedge ("the relationships an invariant *captures*"): the sharpened Representation stratum lets us say plainly that the structural invariant *is* the real regularity.

**Reconciliation with §0.4 (`inv.epistemology.139`) — access and codification.** §0.4 defines a structural invariant as *"a persistent representation of previously validated reasoning."* That is **technically true**, and it does not conflict with "the invariant is a real regularity" (this §0.7): the invariant *in reality* (gravity) is not a representation, but a structural invariant is **accessed and codified by any intelligent system only as a representation** — a system never holds the raw regularity, only a representation of it. So the invariant *as held by a system* (139) is a representation of a regularity that *in itself* is real (144). Two things, not three: the **real invariant** and its **representation** (which serves as both the codified held form, §0.4, and the interface, §0.5/§0.8). Consistent with `inv.reasoning.085` / `inv.constitutional.126` and `inv.reasoning.142`. *(Canonical: `inv.epistemology.144`; the bridge is `inv.epistemology.150`.)*
...
**Operational truth, not absolute truth (the register of every definition in §0).** All three strata are themselves invariant *fields* (`inv.reasoning.081`), and in an **absolute** sense these fields **overlap** at their boundaries — where one ends and another begins is a matter of **magnification and perspective**. The clean tables and diagrams here are **operational** truths: sufficient and sharp at the working magnification, not claims of absolute, non-overlapping boundaries. This is the same register in which standing operates (§0.6, `inv.constitutional.145` — operational confidence, not absolute truth). The definitions hold **operationally**, which is what the discipline requires; absolute cleanliness is neither claimed nor needed, and reading these separations as absolute is a false precision the doctrine explicitly disclaims.

This operational register is not new to the programme — it is the Polity Papers' **FACT: a Fundamentally Accurate Current Truth** (operator-attributed; the Polity Papers are DVN-receipted in the Qriptopian Cartridge, not yet ingested into this register — see the pending canonization exercise). "Truth" in every §0 definition is *Fundamentally Accurate* and *Current* (operational, revisable as evidence and magnification change), never *Absolute*. `inv.epistemology.149` names that register; canonizing the Polity Papers' FACT corpus into this register is tracked backlog. *(Canonical: `inv.epistemology.149`.)*
