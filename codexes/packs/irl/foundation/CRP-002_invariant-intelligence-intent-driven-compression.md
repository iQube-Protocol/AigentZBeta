...
Not a **fact**, not **knowledge**, not **truth** — *previously validated reasoning*. This is the criterion that also grounds IRL Principle 005 (`inv.epistemology.133`). Note the precise scope: "persistent representation" here names the **held, reusable form** a system carries; the invariant it expresses is a real regularity anchored in reality (§0.7), and the *interface* through which a system reaches it is a representation in the §0.8 sense — three distinct things (real regularity · held form · interface), reconciled in §0.7. *(Canonical: `inv.epistemology.139`.)*
...
**Reconciliation with §0.4 (`inv.epistemology.139`).** §0.4 defines a structural invariant as *"a persistent representation of previously validated reasoning."* That names the **held, reusable form** an intelligent system carries — the encoding of the validated reasoning through which the system reaches the invariant. It does not make the invariant *itself* a representation: the real regularity (gravity) is anchored in reality (this §0.7 / `inv.epistemology.144`), the held form is its reusable encoding (§0.4), and the interface between a system and the regularity is a representation (§0.5/§0.8). Three distinct things, one word "representation" used precisely for the middle one. Consistent with `inv.reasoning.085` / `inv.constitutional.126` (natural/discovered vs constitutional/ratified) and `inv.reasoning.142` (the invariant is not its representation). *(Canonical: `inv.epistemology.144`.)*
