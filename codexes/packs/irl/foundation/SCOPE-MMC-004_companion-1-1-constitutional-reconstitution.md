...
```
Avatar   Wallet   Agent Me   Search   Workspace   Overlay
```
...
| **Workspace** | Exposed as a Companion mode. **No workflow redesign** |
...
| **D-11** | **The Copilot-as-shell merge (§3.2).** C1/C2 shipped the Companion as a container with its OWN bottom navigation, stacked above the Copilot's existing nav-menu footer | **PARTIALLY BUILT.** Done: Quick Links render in the copilot's own single-row carousel above the composer and drive the browser (§3.2.5a); `Search`/`Workspace`/`Overlay` migrated into the copilot's menu row via `navExtras` — **additively**, so the copilot keeps the avatar/chat toggle and wallet launcher it already owned (`footerContent` would have replaced them). The split between copilot-native and migrated items is **derived** from the vocabulary (`COPILOT_NATIVE_NAV_ITEMS` / `migratedNavItems()`), never listed. **Ratified: the added modes are COMPANION-DEPLOYMENT-CONTAINED** — nothing but the Companion passes `navExtras`, so no other `CodexCopilotLayer` mount changes (§3.2.6a). **The copilot now NEVER unmounts**: every surface (and the pre-1.1 rail) renders into its `bodySlot`, so the copilot's header, chip carousel, composer and menu row persist across surface switches — and so does the conversation, which the old ternary tore down on every switch (a silent D-8 breach). **The Companion's bottom bar is deliberately KEPT** per the operator ("keep them in both rows … until we test they are working in the copilot menu"). Remaining: (a) retire the bottom bar once the copilot menu is confirmed; (b) header carries R/T dots right + persona left | **Open — BLOCKING C4** |
| **D-15** | Activating Search / Workspace / Overlay unmounted the copilot, taking the migrated navigation with it — "we are not able to navigate back and forward between tabs once one has been activated" | **RESOLVED (2026-07-26).** The surfaces were branches of a ternary whose *fallback* was the copilot. They are now the copilot's `bodySlot`, mounted inside its chrome. This is what makes "the Copilot is the shell" (§3.2) literally true rather than a description of the default surface only. Icon spacing in the copilot's right-hand cluster also loosened from `gap-1` to `gap-2` — still right-aligned, no longer a dense block | **RESOLVED** |
| **D-12** | Search's entry bar is *positioned and shaped* like the composer but is not the composer | **Open.** Reusing the actual composer needs a shared seam extracted from `CodexCopilotLayer`. Recorded rather than claimed: the current state is bottom-placement with matching styling, and calling that "reuses the chat composer" would be the false-parity claim these canaries exist to catch | **Open** |
| **D-13** | `quickPrompts` was **declared** on `CodexCopilotLayer` and never destructured or rendered — the Companion passed Quick Links into a prop that went nowhere, and no carousel appeared | **RESOLVED (2026-07-26).** The prop is now live: destructured, rendered as chips in the **one** existing row above the composer (never a second strip), with `skipInference` routing a selection to `onPrompt` so a Quick Link navigates instead of asking the copilot about the destination. The canary now asserts the whole chain — declared → destructured → rendered — because the previous one grepped the *pass-site* and went green while the citizen saw nothing | **RESOLVED** |
| **D-14** | The wallet rendered at two different widths depending on which route reached it: the Companion's own Wallet item mounted it with `embeddedWidth="fill"`, while the wallet reached **through the copilot** used a hardcoded `"fixed"` cartridge-sized column | **RESOLVED (2026-07-26).** `walletEmbeddedWidth` is now a prop (defaulting to `"fixed"`, so every existing cartridge mount is unchanged) and the Companion passes `"fill"`. In fill mode the rem cap is dropped and the copilot yields the pane while the wallet is open — sharing works in a cartridge, which has room for two columns; in a Companion pane it would squeeze both. One capability, one width, whichever route reaches it | **RESOLVED** |
