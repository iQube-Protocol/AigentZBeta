...
**Onboarding posture (operator direction, 2026-07-18):** the staged split is the frame for the *wider* IRL OS population (read/replicate → graduate to delegated submission). For the **countersigned EXP-P1 partner, Phase 2 is the entry point** — the x409 agreement that freezes the protocol IS the act that authorizes their agent's submission, collapsing sign-off + delegation + submission capability into one constitutional handshake and bringing the partner into the constitutional operating model from day one. This doesn't gate the timeline: the signature uses already-built primitives (at freeze), while the submission endpoint is only exercised once runs complete — so the endpoint build never blocks getting started (CFS-042 §9).
