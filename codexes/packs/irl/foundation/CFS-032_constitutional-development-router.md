...
- [x] **§4 Constitutional Acceptance + §5 Standing wiring — BUILT 2026-07-16, same day** (operator: "Let's build them and close the loop"). Delivered: `capability_registry` table (migration `20260716000000`, RLS service-role-only, + the activity_receipts CHECK rebuilt with the two new action types); `services/constitutional/capabilityRegistry.ts` (pure `buildCapabilityObject` — kind `'capability'` added to `ConstitutionalObjectKind`, T2-safe by construction, enters at the EXPERIMENTAL band 0.25; `registerCapability` — idempotent acceptance ceremony, receipted `capability_registered`, DVN-anchorable; `recordOperationalValidation` — the §5 accrual trigger, HONEST REFUSAL when unregistered (the eligibility gate, HTTP 409), +0.1 per validation capped at 0.55 so accrual reaches `validated` but NEVER `canonical` — canonical requires a Law XI ceremony; `registeredCapabilityBlock` — the loop-closing Gap Analysis read); route `/api/constitutional/capability-registry` (admin-gated, canonical-assets pattern); `buildStageGroundData` folds accepted capabilities into the gap-analysis + context-assembly ground so the NEXT capability request discovers them; Deployment Authorization capsule gains the Constitutional Acceptance section (accept-into-registry post-merge + per-capability operational-validation with evidence ≥10 chars + the live capability ledger). Canary `tests/capability-registry.test.ts` + an executed smoke run (8/8 checks: experimental floor, provenance carriage, determinism, T2-safety, accrual sequence 0.25→0.35→0.45→0.55 cap, validated-not-canonical). **Operator setup: run migration 20260716000000 in Supabase** — until then every write soft-fails with an honest note. What remains unbuilt from this spec: the Development Router itself (§2 — no second executor adapter exists) and automated Operational Validation feeds (today's evidence is operator-attested text; a Chrysalis-Test-driven trigger is a named follow-on).
