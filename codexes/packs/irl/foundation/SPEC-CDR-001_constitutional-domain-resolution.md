...
**Financial context modules** — rendered only when the resolved profile names them. **P4 ratification (operator P4-1/P4-2, 2026-07-25):** the profile names modules explicitly as a typed `CapabilityModuleId[]`; modules are assertions under the profile's own provenance and verification lifecycle, and **do not imply execution-domain membership or executability**. Governance modules render only when explicitly named — never by default, never inferred from `overlayContext`:
