...
## Additions — CFS-035 discovery-ranking dimensions (2026-07-18)

*(The four constitutional dimensions the CFS-035 discovery-ranking Invariant Decision Node projects, extracted from the `scoreCapsule` magic numbers in `app/api/runtime/capsules/route.ts`. Context `discovery`/`ranking`. `proposed` until validated; while proposed the node's `deriveDimensionWeights` returns all-1 (faithful) so the projection equals the incumbent sum — the shadow→authoritative flip becomes meaningful only once these earn standing. This is the honest flywheel: the projection diverges from the magic numbers exactly when the governing invariants have been ratified and used.)*

134. *(reasoning · principle)* **Discovery importance** — an explicitly published, deployed, or resolved experience is more consequential to surface than unpublished or unresolved catalogue content; the operator's own act of publishing is a constitutional signal of importance. *(Extracted from scoreCapsule +10 `sourceType==='experience'` / +6 `assetStatus==='resolved'`.)*
