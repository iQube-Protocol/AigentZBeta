...
1. **The guided-onboarding capability** — **DONE (2026-07-18), in code:** `services/constitutional/guidedOnboarding.ts` (pure plan builder + graded-proof derivation + recommended bounded authority + IRL OS passport deep links) served by `POST /api/constitutional/guided-onboarding`, with the human-authorizer safeguard locked by a canary (`tests/guided-onboarding.test.ts`). The playbook form (`CFS-043a_guided-onboarding-script.md`) documents the same steps. The capability composes the *existing* agreement endpoints (`/api/constitutional/agreement` form/accept/authorize) — no new trust primitive — and is **directly reusable by the Constitutional Financial Services programme** (pass `risk:'money-moving'` → strong-proof requirement + spend-ceiling authority).
