...
## Amendment — The three functions of Invariant Intelligence: compression, orchestration, governance (2026-07-18)

A further round with Aletheon (the hydrogen/valence and personhood/voting examples, reasoning-compression re-examined under the external reviewer's pressure) produced one clean synthesis worth ratifying as a **lens** — not new doctrine. The operator's note on receiving it is the frame: *"this was the foundational insight for me from the earliest days of invariant intelligence emerging as a thing."* Aletheon's own conclusion agrees: the phrase "reasoning compression" was **already ratified** (CFS-000 §1, CFS-008); what the exchange clarified is *where the compression resides* and how three already-ratified functions separate cleanly. This amendment records the separation; it re-ratifies nothing.

**The three functions (each maps to already-ratified canon):**

| Function | What it is | Where the canon already states it | Layer |
|---|---|---|---|
| **Reasoning compression** | The one-time transformation of expensive reasoning into a reusable invariant. The compression happens **once, at discovery/validation**, and lives in the **structural invariant substrate** — not in any later use. | CFS-000 §1 (the three compressions); CFS-008 §7 ("preserving validated reasoning so that it need not be repeatedly rediscovered"; canonical invariants as "pre-paid reasoning"). | I |
| **Reasoning orchestration** | Intent projects the substrate onto the **minimal set of structurally-determining invariants** a purpose requires; assembly, not re-derivation. | CRP-002 (intent as a projection operator onto the minimal governing set); IRL-011 §6.3 (`K*` — "the object the entire discipline optimizes"). | I → runtime |
| **Reasoning governance** | The runtime and constitutional invariants that discover, validate, version, compose, and govern the substrate over time. | CFS-019 §2 (Layers II/III); the 2026-07-17 amendment. | II / III |

**The locus correction, restated (already ratified 2026-07-17 + EXP-010 §7B — cross-referenced, not re-ratified):** the runtime does **not** create the compression. The compression is in the substrate; the runtime *reuses, orchestrates, and governs* it. Aletheon's earlier "the runtime compresses reasoning" was imprecise; the precise form is **an invariant is the persistent representation of previously-validated reasoning, reusable across a class of intents** — a cache of pre-computed reasoning primitives (CFS-008 §7; CFS-000 §1 "inherit the compressed expertise of every prior validated reasoning act"). The efficiency claim is therefore **computational-complexity, not context-engineering**: the system stops re-deriving stable structure (CFS-008 §7; EXP-010 Claim A vs Claim B, §7B).

**Projection is causal, not correlative (the lexical sharpening).** The orchestration function selects the invariants that **structurally determine** the behaviour relevant to the intent — not the ones that merely correlate with it, and not the object's identifying information. The operator's formulation: *"you don't need the unique identifying factors; you need the factors that relate to the intent"* — sharpened to **behaviour-preserving abstraction**: *you do not need the identity of the object, you need the properties that determine its behaviour relative to the intent.* The valence-vs-atomic-mass and personhood-vs-passport-number examples are the illustrative cases. This extends CRP-002's "governing invariants" with the causal/correlative distinction and binds it to the intent: the projected set (the "Venn overlap") changes shape entirely with the intent, and for some intents the identity **is** the determining property — projection is intent-relative (CRP-002 §"intent-relative compression").

**What this amendment does NOT do:** it mints no invariant (the pieces are already carried by `inv.reasoning.081–085`, `inv.epistemology.132/133`, and CRP-002); it does not rename "Knowledge Compression" (the product-named operating-model stage is deliberate and already reconciled — CFS-008 opening note; CFS-006a, the operating-model rename); and it changes no experiment beyond the minimal-sufficiency probe added to the parked Hypothesis A (EXP-010 §7B). The three functions are a reading of the existing layers, offered so future drafting keeps compression (substrate), orchestration (projection), and governance (runtime) from collapsing back into one register.

**Ratified 2026-07-18 by operator direction**, in dialogue with Aletheon; grounded on CFS-000 §1, CFS-008, CRP-002, IRL-011 §6.3, and the 2026-07-17 layer-separation amendment.
