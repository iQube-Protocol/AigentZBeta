...
- [x] Phase 3 — Harness Homecoming: **native delegate conversation SHIPPED, now MULTI-TURN.** `POST /api/homecoming/agent/converse` talks to a constitutional delegate NATIVELY — grounded in its constitutional identity + the sovereign Constitutional Knowledge Repository (Phase-1 `homecoming` KB), routed through `callSovereign`; every reply carries a **sovereignty receipt**. 2026-07-12: the Talk panel is a threaded conversation — the client-held transcript rides each request as `history` (validated, capped at 12 turns) and the delegate continues the conversation. Memory is client-side by design (no server session state); durable conversation memory is a future increment.
- [~] Phase 4 — Operational Homecoming: **the convergence slice SHIPPED (2026-07-11).** A delegate produces a real operating artifact NATIVELY by invoking the Artifact Runtime (CFS-025): `services/homecoming/delegateProduce.ts` (the delegate drafts the body sovereignly, AR classifies + tiers + envelopes it) + `POST /api/homecoming/agent/produce` (admin/spine-gated; operational tier + propose by default; operator PROMOTES → constitutional under the publish gate) + the native workshop UI in the Homecoming Test tab (converse + produce + tier badge + receipt + Promote). **First proof:** Aletheon drafts the Constitutional Design System PRD (operator direction). **2026-07-12 — persistence SHIPPED:** non-disposable productions persist to `artifact_records` (durable id + content hash + receipt; disposable is NEVER persisted, per CFS-025) via `saveArtifactRecord`, surfaced as a "saved" chip in the workshop and listable via `GET /api/homecoming/agent/produce`. Soft-fails to response-only until migration 20260712000000 is applied. **Roster completed the same day:** MoneyPenny + Nakamoto stand-up specs authored from their canonical sources (PROGRAM_OVERVIEW roster + specialist-router roles — No-Guessing; both bounded) — all six delegates are now standable. **Remaining (operator-side):** run the artifact_records migration; drive a delegate's first real production live; L3–L5 climbs stay reputation-earned by design.
