...
| **Invariant Field Explorer** | `/api/invariants`, `/api/research/invariant-field` (T2-safe, read-only/pure) | 401 | **ELIGIBLE — SHIPPED 2026-07-17.** The 415-line spine-gated field route was extracted into a shared module (`services/research/invariantFieldQuery`) that BOTH the gated route (now 57 lines: gate + delegate) and a new public route (`/api/public/irl/invariant-field`) call — no duplication, gate untouched. `publicMode` on `InvariantFieldExplorerTab` points its fetches at the public routes (token-free). Both GET (neighbourhood/overview) and POST (pure counterfactual what-if) are safe — read-only, no LLM/credits. |
| **Dashboard** | `results`, `research/overview` (401), **`constitutional/chrysalis-test` (403, admin — a live credit-touching test)** | 401 + 403 | **PARTIAL — SHIPPED 2026-07-17.** New public projections `/api/public/irl/experiments-results` + `/api/public/irl/research-overview` (both reuse shared readers extracted into `services/research/publicReads`; gated routes untouched). `publicMode` on `IRLDashboardTab` uses them and OMITS the admin/credit-touching chrysalis-test source entirely (the section renders an honest "operator surface" note). Public visitor sees mission + published results + the live lifecycle. |
| **Research Copilot** | `research/objects` (**admin write**), overview, results, **live LLM chat (spends credits)** | 401 + 403 | **SUBSCRIPTION-GATED (reframed 2026-07-17).** Not "permanently internal" — the Research Copilot becomes the **Researcher pathway's** copilot tier (a fifth route into the Founder Office, peer to the developer pathway's DevOn/Aigent Z copilot), gated behind subscription/cohort access rather than admin. Its live LLM chat + credits are appropriate for a paid tier, not anonymous browsing. See CRP/CFS follow-on for the Researcher-path subscription integration. |

**Shipped 2026-07-17:** the public Invariant Registry (Browse), the public Invariant Field Explorer, AND the public Dashboard — three live read-only instruments in IRL OS. An anonymous visitor can browse the real substrate, explore the live field with counterfactual projection, and see the mission + published Foundational Validation Series results + live lifecycle. The Research Copilot is reframed as the subscription-gated Researcher-pathway tier (below), not permanently internal. The audit converted "four instruments, safety unknown" into three shipped public + one moved to a paid tier.
