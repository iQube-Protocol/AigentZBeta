...
**Shipped 2026-07-17:** the public Invariant Registry (Browse) AND the public Invariant Field Explorer — two live interactive instruments in IRL OS. An anonymous visitor can now browse the real substrate (Registry) and explore the live enables/constrains/contradicts field with counterfactual what-if projection (Field Explorer). Remaining: the Dashboard (partial — needs public results/overview projections + the admin chrysalis-test source dropped) and the Research Copilot (permanently internal). The audit converted "four instruments, safety unknown" into two shipped + one scoped + one closed.
