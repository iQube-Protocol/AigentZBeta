...
- **Minimal-sufficiency ablation (added to Hypothesis A's design, 2026-07-18 — Aletheon's proposal).** Beyond comparing answer quality across the three representations, add a **removal** probe that operationalizes `K*` (IRL-011 §6.3 — the smallest curated set of invariants that solves a task at fidelity φ). For each representation, incrementally **remove units** (invariant statements vs summary sentences vs corpus passages) and measure *how much can be removed before reasoning catastrophically fails*, and *which removals cause the collapse*. The distinctive prediction: the invariant substrate should **preserve reasoning with fewer removal-tolerant units** than a conventional summary — i.e. its remaining units are the structurally-determining (causal) ones, so each removal is more load-bearing, and non-determining information is already absent rather than padding the representation. If invariant representations consistently reach the reasoning-break point having carried *less* structurally-required information than topical summaries, that is evidence for **behaviour-preserving abstraction** (CFS-019 "three functions" amendment) — a stronger result than "better context," because it isolates causal sufficiency from topical relevance. C ≈ (summary) on minimal sufficiency = the abstraction claim is unsupported at current scale — publishable either way (Principle 004). Charter alongside Hypothesis A's arms; no EXP number minted here.
