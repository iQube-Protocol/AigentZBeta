...
6. **Collection-size guard + enlargement discipline (locked at freeze).** The **fixed Arm C slice ⊆ 40% of `Crystal vP1`** — a genuine subset, so Arm B's per-task live selection retains discriminatory power and the selection-neutral/selection-sensitive split (§5, P-IRL-2) has a domain to be tested on (else Arm C ≈ Arm B degenerately). **The crystal must be sufficiently large for Arm C to remain a proper subset AND for live selection to retain discriminatory power at the task set's scale. The exact collection size is an IMPLEMENTATION PARAMETER determined by those experimental constraints — it is NOT a number fixed a priori.** (Worked illustratively: at the current 18-invariant collection the ⊆40% guard caps the slice at 7 statements — plainly insufficient to ground 24 tasks incl. 12 derivation items, and it leaves Arm B nowhere to differ from C; so the collection must grow before it can carry this task set. The target size follows from the constraints and the finalized task set, not from a chosen figure.) `Crystal vP1` is enlarged before freeze under four conditions (violating any re-imports the affinity problem §12 pre-registers):
   - **(a) Actual-state freeze, no synthetic promotion** *(AMENDED 2026-07-28 by operator ruling; supersedes the original `proposed → validated` prerequisite).* New invariants may enter the experimental crystal **in their actual lifecycle state, including `proposed`**. They must not be bulk-promoted, assigned synthetic validation counts, or granted synthetic Standing to satisfy an experimental population target. The freeze **records status, validation counts and Standing as observed**. All experimental arms must draw from the same declared eligible population unless lifecycle status or Standing is an explicitly pre-registered experimental variable.
     *Why the original wording was wrong:* it read as a prerequisite — that an invariant must reach `validated` before it can participate — which contradicts the mixed-status corpus (the Seed Corpus alone carries 144 `proposed` members of 373), contradicts the point of recording lifecycle fields in the snapshot at all, and contradicts this experiment's own purpose of *generating* validation evidence. What it was actually protecting against is **filler**: padding the population with synthetic counts. That prohibition stands and is restated above.
     *The asymmetry it named is real, and is fixed in the selection rule instead.* Arm B ranks standing-primary (`services/invariants/grounding.ts::rankByStanding`) while Standing derives from `times_validated` (`services/invariants/lifecycle.ts`), so zero-Standing members enlarge Arm C's denominator while being practically unreachable by Arm B. **Standing must not determine corpus ELIGIBILITY. It may remain part of an experimental arm's declared retrieval treatment.** *(Corrected 2026-07-28: an earlier draft of this amendment said standing-primary suppression is "disabled for P1", which was too broad — Arm B is defined as IRL's complete live runtime, so stripping its ranking would destroy the ecological validity the arm exists to provide.)*
     The corrected design: **both arms draw from the same frozen eligible population.** Arm B uses the live standing-weighted ranking *within* that population; Arm C receives its pre-registered fixed slice from the *same* population. The protocol records standing-weighted accessibility as **part of Arm B's treatment, not as a corpus-membership rule** — which removes the asymmetry (Arm C's denominator can no longer be enlarged by members Arm B cannot reach) without pretending the arms share identical selection mechanics. Standing-weighted vs non-standing-weighted retrieval remains a legitimate question for a **separate later experiment**; it may not enter P1 as an unpre-registered confound.
   - **(b) Sequence is sacred.** enlarge → **freeze `Crystal vP1`** → construct the fixed slice by the standard domain procedure → **hash both** → *then* the reviewer builds tasks. Task construction must never precede the freeze, or invariants get authored to fit the tasks and the affinity limitation returns through the side door. The reviewer sees the **domain corpus boundary**, not the crystal contents, when writing tasks (§5.1).
   - **(c) In-domain for Phase 1.** Enlargement remains within the defined experimental domain (per the §12 limitation). Whether cross-domain widening is warranted is itself a question for a *successor* experiment (cross-domain transfer), not a Phase-1 design choice.
   - **(d) Derivation headroom.** The 12 derivation tasks need invariants whose **conjunctions entail unstated conclusions** — relational and conditional statements, not isolated atomic assertions — or the generative-sufficiency probe (P-IRL-3) has nothing to measure and dies of malnutrition rather than falsification.

   **Reviewer recommendations vs protocol amendments (IRL-016).** The sizing rationale here was informed by the independent reviewer and is adopted as a *constraint principle*, not as reviewer authorship of the design — protocol design remains the originating team's responsibility; the reviewer evaluates, challenges, and replicates (`inv.reasoning.350`). The pre-registration bundle (§10) records the slice fraction, the final crystal size, and per-invariant validation counts.
