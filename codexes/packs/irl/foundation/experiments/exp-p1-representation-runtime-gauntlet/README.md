...
6. **Collection-size guard + enlargement discipline (locked at freeze).** The **fixed Arm C slice ⊆ 40% of `Crystal vP1`** — a genuine subset, so Arm B's per-task live selection retains discriminatory power and the selection-neutral/selection-sensitive split (§5, P-IRL-2) has a domain to be tested on (else Arm C ≈ Arm B degenerately). **The crystal must be sufficiently large for Arm C to remain a proper subset AND for live selection to retain discriminatory power at the task set's scale. The exact collection size is an IMPLEMENTATION PARAMETER determined by those experimental constraints — it is NOT a number fixed a priori.** (Worked illustratively: at the current 18-invariant collection the ⊆40% guard caps the slice at 7 statements — plainly insufficient to ground 24 tasks incl. 12 derivation items, and it leaves Arm B nowhere to differ from C; so the collection must grow before it can carry this task set. The target size follows from the constraints and the finalized task set, not from a chosen figure.) `Crystal vP1` is enlarged before freeze under four conditions (violating any re-imports the affinity problem §12 pre-registers):
   - **(a) Normal receipted lifecycle.** New invariants pass `proposed → validated` with real receipted validation counts (the EXP-009 accrual discipline) — never bulk-authored to hit a number. Arm B's selection and any standing behaviour read those fields; zero-validation filler distorts the arms asymmetrically.
