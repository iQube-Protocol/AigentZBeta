...
6. **Collection-size guard + enlargement discipline (from the Austin review — locked at freeze).** The **fixed Arm C slice ⊆ 40% of `Crystal vP1`** (else Arm B's per-task live selection cannot differ from Arm C's fixed slice, the selection-neutral/selection-sensitive split (§5, P-IRL-2) has no domain, and Arm C ≈ Arm B degenerately). **Sizing, worked backwards from the task set (reviewer spec, banked as a binding term):** 24 tasks incl. 12 derivation items need a slice of ~**20–24 invariants** to ground them; the ⊆40% guard then floors the crystal at **≥ 60 invariants (target 60–90)**. At the current 18-invariant doctrine collection the guard caps the slice at **7 statements** — insufficient to ground the task set, and it leaves Arm B nowhere to differ from C, so the classifier degenerates and **P-IRL-2 becomes untestable**. Enlargement is therefore not merely permitted by the guard; it is what makes the Institute's own prediction falsifiable. **`Crystal vP1` MUST be enlarged to ≥ 60 invariants before freeze**, under four conditions (violating any re-imports the affinity problem §12 pre-registers):
   - **(a) Normal receipted lifecycle.** New invariants pass `proposed → validated` with real receipted validation counts (the EXP-009 accrual discipline) — never bulk-authored to hit the number. Arm B's selection and any standing behaviour read those fields; zero-validation filler distorts the arms asymmetrically.
   ...
   - **(c) Deepen, don't widen.** Enlargement stays in the SAME domain (per the §12 limitation). A two-domain crystal makes the fixed slice bloated or insufficient and muddies the classifier.
   - **(d) Derivation headroom.** The 12 derivation tasks need invariants whose **conjunctions entail unstated conclusions** — relational and conditional statements, not 42 isolated atomic assertions — or the generative-sufficiency probe (P-IRL-3) has nothing to measure and dies of malnutrition rather than falsification.

   The slice fraction, crystal size, and per-invariant validation counts are recorded in the pre-registration bundle (§10).
