...
4. **Record run (10 intents × 3 reps):** stability 1.0 · compression 0.65 · coverage 0.21 mean · novelty 0.75. **These IRV figures are UNAFFECTED by the IRE-6 coordinate-scale defect** — every one is computed from resolved seed ids and their statements, never from a coordinate value, and slice membership is standing-ranked server-side *before* calibration runs. **IPV (10 × 5 reps): standing-weight + seed-set reproducibility 1.0 stand; the coordinate-weight figure is a *pre-fix diagnostic; invalid for confirmatory comparison and not numerically comparable with post-fix runs* (operator ruling 2026-07-27) — see `../ipv-001-invariant-projection-validation/README.md`.** **Verdict:** the IRE is a deterministic instrument with meaningful on-domain alignment that has survived adversarial calibration of its measurement methodology; its one pathology is fixed. **Instrument validated for EXP-P1** on the stability + no-pathology + relevance basis (coverage a reported proxy, not the gate). Reviewer note: `../exp-p1-representation-runtime-gauntlet/STAGE-0_HANDOFF.md`.
