...
ISR-001 results publish through the **same canonical, receipted pipeline** as every other experiment. Phase 1 (internally executed) publishes via the admin door `POST /api/experiments/results`; Phase 2 (passport-delegated external submission) publishes via `POST /api/experiments/results/external` under a Polity Passport + bounded `DelegatedAuthority` + an authorized x409 Constitutional Agreement, `origin:'external'`, honestly labelled `independently submitted` vs `internally executed` (CFS-042 §2, §7). **Prerequisite (CFS-042 §8):** the `EXPERIMENTS` allow-list (`app/api/experiments/results/route.ts`) and the `publishResult.ts` experiment-id union must be widened to include `ISR-001` before any result can publish through either door — flagged here so it is not discovered at runtime. Verification stays trustless: anyone recomputes sha256 over the verbatim results JSON and compares to the anchored hash; external origin changes *who wrote it*, not *how it is verified*.
