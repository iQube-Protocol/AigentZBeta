...
**Status:** Architectural Foundation — **RATIFIED 2026-07-28** by operator act `ACT-IRE-FAMILY-2026-07-28`, decision id `CFS-037`.
...
| Field object | **Exists** | `FieldSnapshot {stampedAt, context, slice, citedIds}` — declared in `services/invariants/grounding.ts` since the IPE binding of 2026-07-27 (`f7ab22432`); `engine.ts` re-exports the TYPE only. The 2026-07-17 inventory recorded it in `engine.ts`, which is where it then lived | **Extend** → the Resolved Constitutional Field adds resolved intent + coordinates (§6) |
...
- [x] **RATIFIED 2026-07-28** — authored from the operator/Aletheon PRD-IRE-001 + pre-reasoning dialogue, reconciled against a code-truth inventory. Ratified in full — the numbered items below are what the operator approved: (1) the IRE/IPE separation + CFS-035 rename to Invariant Projection Engine; (2) the Universal Invariant Library baseline (§4); (3) the three coordinate classes (§5); (4) the four-C lifecycle (§7); (5) the Phase-0 shadow build (§12).

  Authority: operator act `ACT-IRE-FAMILY-2026-07-28`, decision id `CFS-037`, authority basis **Law XI — amending canon is an operator act**. Frozen content hash `sha256:8bc918f6a3710fded1133a248dad064f1194d7fb7f6243764070cdd05032b511` (bytes after the IRE→IPE consistency corrections, before this record was written). Ledger row: `codexes/packs/polity-core/items/AMENDMENT_RECORDS.md`. The 2026-07-17 agent transcriptions are corroboration of intent, never the authority (Law XI).
