...
1. **`codexes/packs/irl/foundation/IRL-010_constitutional-runtime-technical-specification.md`** — the Constitutional Runtime specified from the code, RFC-style:
   ...
   - Part VII implementation-status inventory incl. what does NOT exist (external replication, variance bands, D2 execution).

**Correction (operator, 2026-07-14, same day):** the first draft listed "identity continuity" as planned — wrong framing, corrected in place. The continuity primitive EXISTS as **personhood continuity** (Law XIII, CFS-009: personhood → individualization → standing; identity an optional derivative of individualization): World ID personhood proof with persisted nullifier, Kybe DID continuity root held T0, outward existence only as T2 alias commitments, `kybeAttestation` via explicit `discloseCredential()` only, Polity Passport lifecycle, standing accruing to the continuous subject. IRL-010 gained §2.11 (formal definition) + the corrected Part VII row; IRL-010A gained row 1.5a with the correction noted transparently, and names the follow-on: register Law XIII's statement as a canonical `InvariantRecord` (constitutional namespace) so the ratified law also lives in the canon.
