...
**Update 2026-07-25 — Overlay generalised beyond the two-shape whitelist (operator-directed, "1 and 2 for unmapped pages + 3 as you suggest").** Live-verifying the deep-link fix above surfaced the operator's next question directly: why is Overlay whitelisted by domain at all when Capture ("Pull Across") works on any page? The answer, traced through the code rather than asserted: Capture never classifies a page — it hands consented raw content (URL/selection/page text) to the intent pipeline, so it generalises trivially. Overlay composes a specific, hand-built dashboard per page type (`github-repo`: registry match by repo name; `banking`: standing + capability match) — genuinely composed views over real backend data, not text extraction. The domain table was never a security gate; it was "which dashboards have I actually built," and forcing an unmapped domain into an existing shape would mean fabricating data that isn't real for that page (`google.com` was explicitly named as the case this rule exists to refuse).

Rather than inventing new hand-classified shapes with no real backing data source (there is no email-inbox or generic-document composition target today — building one for the sake of "more shapes" would repeat the exact fabrication risk the two-shape design exists to avoid), the three approved items collapsed into one honest generalisation:

1. **A `'generic'` fallback for any unmapped-but-observed domain** (`composeGenericOverlayCard`, `services/companion/overlayComposition.ts`) — a best-effort registry/research match using the page's own title, via the EXACT SAME federation functions (`searchRegistryIQube`/`searchRegistryAsset`/`searchResearch`/`rankSearchResults`) the github-repo card already calls, just with the page title as the query instead of an extracted repo name. Never a second matching implementation.
2. **Persona-level data (standing, delegations) now renders on every page, not just banking-shaped ones** — item 3 of the operator's ask. These were never page-specific; hiding them on an unmapped domain was an artifact of the card being all-or-nothing per shape, not a boundary worth keeping.
3. **The route's four empty-state reasons stay split.** Only `domain-unmapped` (a real, currently-granted domain with no shape) composes the generic card. `no-observation` / `no-domain-observed` / `grant-revoked` are consent problems, not classification problems, and still return the honest empty state — composing a card there would use an observation the route has no legitimate basis to act on.

The panel's "Pull Across" mention on the generic card is text, not a button: there is no message path from the side-panel UI to the context-menu-only capture flow, so a button would be a fabricated affordance that does nothing on click. Canary: `tests/companion-observer.test.ts` asserts `composeGenericOverlayCard` degrades to no standing signal (Supabase absent) and no fabricated matches (no page title) rather than erroring or inventing data.

**Bug found and fixed same day, live-verifying the generic card on Gmail.** The operator flagged Gmail as "probably the most-used registry service" and was concerned by an empty "Related in the registry" section there, worried the registry sweep was incomplete. It wasn't — `Gmail · Create draft` / `Gmail · Send` are real, already-registered connector assets (`services/google/connectors.ts`). The actual cause: the generic card's search uses the page's own title as its query, and `rankSearchResults`' `matches()` requires the ENTIRE query string to appear verbatim in a title/description (`searchFederation.ts:90`). Gmail's tab title is the inbox content ("Inbox (72,138)"), never the word "Gmail" — so the query was wrong for that page, not the registry incomplete.

Fix mirrors `repoNameCandidateFromTitle`'s existing pattern, one level up: `domainSearchHint` (`overlayMapping.ts`) is a small, explicit table mapping well-known Google Workspace hosts to their product name (`mail.google.com` → `'Gmail'`, `drive.google.com` → `'Google Drive'`, `docs.google.com` → `'Google Docs'`, `calendar.google.com` → `'Google Calendar'`, `tasks.google.com` → `'Google Tasks'`). `composeGenericOverlayCard` now searches BOTH the domain hint and the title (deduped, domain hint prioritised in the merge) rather than title alone — a second real query, never a second matching implementation. A domain outside the table degrades to title-only search, unchanged from before.

Named, not silently accepted: `docs.google.com` hosts Docs/Sheets/Slides/Forms under one hostname (distinguished only by URL path), so it's mapped to the coarser `'Google Docs'` — a Sheets or Slides FILE page (title = the file's own name) still won't precision-match its specific product. Path-based disambiguation is a real follow-on, not built here. Canary: `tests/companion-observer.test.ts` locks the domain-hint table and the pure candidate-builder (`buildGenericSearchCandidates`) against the exact reported case (Gmail title alone → `[]`; with the hint → `['Gmail', 'Inbox (72,138)']`).
