...
| `extension/companion-observer/background.js` | Modified (additive) | Registers a single "Pull Across → metaMe" context-menu item (`contexts: ['page','selection','image']`) in a `chrome.runtime.onInstalled` listener; `chrome.contextMenus.onClicked` calls `performCapture(info, tab)` directly (see "As actually built" below). `buildCapture` branches on `info.selectionText` (selection) → `info.mediaType === 'image'` (image, using `info.altText` as a content substitute) → a `.pdf`-suffixed `tab.url` (pdf, NO client-side extraction) → else a whole-page capture via `chrome.scripting.executeScript` (reusing the SAME `activeTab` grant the click provided). `performCapture` runs the client-side `assertCaptureRespectsGrants` pre-check, then POSTs to `/api/companion/capture` via the SAME `ensureFreshToken`/Bearer-token + retry-once-on-401 mechanism the Observer calls already use. Transient icon-badge feedback (✓/✗) added for UX, no new permission. |
| `extension/companion-observer/constants.js` | Modified (additive) | Hand-synced `CAPTURE_SOURCE_KINDS` / `SOURCE_KIND_TO_CAPABILITY` / `CAPTURED_CONTENT_MAX_CHARS` / `COMPANION_CAPTURE_API_URL`, mirroring `types/companionCapture.ts` — same flagged hand-duplication risk as the Observer constants above it. |
| `extension/companion-observer/observerConsentExt.js` | Modified (additive) | Adds `assertCaptureRespectsGrants(capture, state, siteDomain)`, mirroring `services/companion/captureConsent.ts` field-for-field — same hand-sync discipline as `assertObservationRespectsGrants`. |
| `extension/companion-observer/content.js` | NOT modified | Turned out unnecessary once built — see "As actually built" below. |

**As actually built (deviation from the sketch above, noted honestly):** `chrome.contextMenus` only exists in the background service worker's own context, so `chrome.contextMenus.onClicked` calls `performCapture(info, tab)` directly rather than round-tripping through a `CAPTURE_REQUEST` runtime message to itself — simpler, identical outcome, no `content.js` changes needed for the three implemented source kinds (selection/image/pdf/webpage all resolve from `info`/`tab` plus, for a whole-page capture, `chrome.scripting.executeScript` reusing the SAME `activeTab` user-gesture grant the context-menu click itself just provided — the same pattern `connectToMetaMe`'s `extractSupabaseSessionFromPage` already established). `content.js` was therefore NOT modified — the plan's proposed `GET_SELECTION_CONTEXT` fallback turned out unnecessary once built. Badge feedback (`chrome.action.setBadgeText`, ✓/✗, no new permission) was added as the minimal UX signal the plan didn't originally specify.

**Live-verification — done, 2026-07-23, per §0.7's established recipe** (`xvfb-run` + isolated `playwright-core` + `/opt/pw-browsers/chromium-1194`, real extension directory, not a throwaway test extension):
1. Loaded the real extension; confirmed the background service worker started (manifest validity confirmation, same reasoning as PRD-MMC-IMPL-001 §7).
2. Confirmed `performCapture`/`buildCapture`/`assertCaptureRespectsGrants`/`extractPageText`/`isPdfUrl`/`PULL_ACROSS_MENU_ID`/`CAPTURE_SOURCE_KINDS`/`SOURCE_KIND_TO_CAPABILITY` exist as callable/live values in the worker's global scope.
3. Seeded an empty grant state; called `performCapture` with a selection capture; confirmed `assertCaptureRespectsGrants` refused it and NO network call fired (mocked `fetch` to detect).
4. Granted `'selection'`; called `performCapture` again with no auth session present; confirmed it passed the consent gate and failed cleanly with `reason: 'no-auth-session'` — the honest, reachable failure mode in a sandbox with no real Supabase session (same limitation §7.1 already named).
5. Seeded a fake auth session + mocked `fetch`; confirmed the POST body was a well-formed `CapturedObject` (`sourceKind: 'selection'`, correct `contentText`/`sourceUrl`/`capturedAt`) and the call returned `ok: true`.
6. Called `buildCapture` directly for a `.pdf` URL; confirmed the returned object has `sourceKind: 'pdf'` + `sourceUrl` and NO `contentText` key at all — confirms the extension defers extraction to the server, never attempting it client-side.
7. Called `buildCapture` for an image context (`mediaType: 'image'`, `srcUrl`, `altText`); confirmed `sourceKind: 'image'` with the alt text used as a title/content substitute.

All 7 checks passed with genuine captured output (not paraphrased assertions), mirroring PRD-MMC-IMPL-001 §7.1's evidentiary bar. NOT verified (same limitation named there): driving the actual native right-click menu via a real user gesture, and any live run against a real, logged-in metaMe deployment.
...
*Build status, 2026-07-23: Increments 1–4 were implemented and pushed under the operator's explicit real-time build authorization ("go") for this session, following this plan's scope exactly as written (Increment 4's small "as actually built" deviation is noted inline above). This is distinct from — and does not substitute for — the formal, line-item ratification record in §6 below, whose per-increment boxes remain unchecked pending the operator's own explicit pass over this document, per this repo's discipline that a document (or a session instruction to proceed) does not self-ratify a written record. Increment 5 (canary tests) landed alongside each of Increments 1–4 rather than as a separate final pass, since this repo's established session pattern ships tests with the code they cover.*
