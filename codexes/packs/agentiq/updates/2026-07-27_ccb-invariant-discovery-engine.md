...
| Canon caveat | **RESOLVED 2026-07-28 — the operator decision this row was waiting on was taken.** The caveat read: *`CFS-048` has no document in `codexes/packs/irl/foundation/`; the engine's header cites it (`services/invariants/discoveryEngine.ts:2`) and eleven other files reference it, but the charter actually lives in the agentiq updates pack.* Ruled a contiguous filing-convention defect across CFS-045…CFS-048 and repaired family-wide: the charter was moved byte-identical to `codexes/packs/irl/foundation/CFS-048_invariant-discovery-engine-charter.md` and registered in `codexes/packs/irl/collections.json`. No stub was left in the updates pack — a pointer would have hidden the convention failure rather than restoring canonical resolution. The phase records (`phase1a`, `phase2`, `parent-linking`, `recursive-compression`) remain in the updates pack, correctly: they are build records, not the charter. `PRD-IDE-002_commercialisation-invariant-discovery.md` was already in the foundation. |
