...
- ~~The real browser-extension manifest, content scripts, and background/service worker — cannot be built in this sandbox~~ — **superseded 2026-07-23, see §7 Increment 6 below.** On operator challenge ("I can understand it can't be verified here but why can't it be built here?"), this claim was corrected: a browser extension is source files (manifest.json, a background worker, a content script) — authoring them is not different in kind from authoring any other file in this repo. What genuinely cannot happen in this sandbox is bundling/packaging it, loading it in a real browser, or verifying its `chrome.*`/`browser.*` API usage against real runtime behavior. The operator then explicitly authorized authoring the source anyway, on that corrected understanding — see §7.
- **Phase 3** (Constitutional Overlay + Universal Search + Notifications, PRD-MMC-001 §6) — not chartered here.
- **Phase 4** (mobile/desktop/VS Code/embedded-widget presentation surfaces, §6) — not chartered here.
- **SmartTriad's Observer "offers" UI** (PRD component 3's suggestion-rendering surface, e.g. "Import this FATF guidance into IRL? Extract invariants?") — depends on a real observation source (Increment 6, §7) existing to originate an offer from; this plan builds the substrate the offer would eventually be grounded on (Increment 3), not the offer-rendering UI itself.
...
- **Any live-DOM/live-tab verification, screenshot, or "tested in a browser" claim** — still true even after Increment 6 (§7): authoring source is not the same as verifying it runs. No claim of a working, tested extension exists anywhere in this document.
- ~~A real Supabase migration run~~ — **done, 2026-07-23**: `supabase/migrations/20260815000000_companion_observer_grants.sql`, promoting the Increment 2 sketch (`_sketch_companion_observer_grants.sql.example`, kept in place, header-annotated as superseded) to a real, runnable migration. Not yet executed against a live database by the operator — that's still their action to take.

---

## §7 Increment 6 — Browser-extension source (operator-authorized 2026-07-23, twice-corrected scope)

**Status: operator-authorized, scope corrected a SECOND time on further operator challenge.** The first correction (above, §4) established that authoring the extension's source is unremarkable. The operator then asked "can this be done from here?" of the four specific verification gaps this document originally claimed — which prompted an actual test rather than a second assertion: a minimal Manifest V3 test extension was written and loaded into the pre-installed Chromium (`/opt/pw-browsers/chromium-1194`) under a virtual display (`xvfb-run`), driven by a small, isolated `playwright-core` install (scratchpad-scoped, not touching this repo's `node_modules`). **Its background service worker actually started** (`chrome-extension://.../background.js` observed as a live service worker via Playwright's `context.serviceWorkers()`). This is genuine behavioral verification — not "written to spec," an actual loaded, running extension in a real browser.

**Corrected capability, precisely:**
- **Bundling is unnecessary** for a plain-JS MV3 extension — Chromium loads an unpacked directory directly.
- **Loading in a real browser IS possible here** — verified, not assumed. Recipe: `xvfb-run -a node <script>.mjs` where the script uses `playwright-core`'s `chromium.launchPersistentContext(userDataDir, { headless: false, executablePath: '/opt/pw-browsers/chromium-1194/chrome-linux/chrome', args: ['--disable-extensions-except=<path>', '--load-extension=<path>'] })`.
- **`chrome.*` API behavior can be PARTIALLY verified** — a live Chromium instance can be driven with a real test page, console output captured, and messaging between content script and background worker exercised directly. What remains NOT verifiable here: a full end-to-end run against a live, logged-in metaMe deployment (no real Supabase session/cookie exists in this sandbox to authenticate against), and anything requiring an actual installed-from-store extension lifecycle (update checks, store review, cross-machine persistence).
- **Manifest validity IS confirmed by this method** — Chromium accepting the extension and starting its service worker without erroring is exactly that confirmation.

This increment therefore both authors the extension's source AND smoke-tests it for real in this sandbox, using the recipe above — a materially stronger acceptance bar than the original (source-only) scope.

**Scope:**
- `manifest.json` (Manifest V3) declaring the minimum permissions the granted-capability model (§4.1's table) actually needs, a background service worker, and content-script injection rules.
- A background service worker that relays messages between the content script and the Companion runtime's API surface (Increment 2's grant routes; a future observation-ingest endpoint this increment may also need to define, since none exists yet from Increments 1–5).
- A content script that, for each `ObserverCapability` the current persona has actually granted (checked against the SAME grant state Increment 2's routes serve — never a second, extension-local notion of "granted"), reads only what that capability permits and populates a `BrowserContextObservation` (Increment 3's type) — never more.
- **The authentication design decision, made explicit here rather than left implicit:** per CLAUDE.md's own documented fact that the spine "ignores cookies entirely for auth" and requires a Bearer token, and per CLAUDE.md's own sanctioned manual pattern for extracting the Supabase access token from `localStorage` (the "Debugging from DevTools" section) — the extension's "connect" step reads that same token from the metaMe web app's `localStorage` (while the user is on a metaMe tab) via a scripted extraction, stores it in extension-local storage, and attaches it as `Authorization: Bearer <token>` on every API call, mirroring `personaFetch`'s own mechanism relocated into the extension. This is a genuine open design question this plan had not previously resolved (no prior increment needed it) — token refresh/expiry handling is explicitly NOT solved by this increment; flagged as an open question, not silently glossed over.
- A minimal popup surface is acceptable if it helps demonstrate the consent-check flow, but is not required to duplicate `ObserverGrantPanel` (Increment 4) — the popup may simply deep-link/open the existing `/triad/embed/companion` page for actual grant management, keeping one grant-management UI rather than two.
