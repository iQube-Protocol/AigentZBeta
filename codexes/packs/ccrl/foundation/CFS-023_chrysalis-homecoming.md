...
- [~] Phase 2 — Agent Homecoming: **stand-up seam SHIPPED.** `POST /api/homecoming/agent/stand-up` runs the existing genesis core (`sponsorPolityAgent`) for a named delegate, seeding its RootDID (card-only archetype L0 → persisted registry identity L1) and reporting the resulting Constitutional Presence. Aletheon is the authored archetype (a bounded, non-autonomous constitutional companion, grounded in its card). Presence scorer corrected to key on `agent_card_slug` so genesis rows (whose `agent_id` is `polity-bound:<slug>`) resolve. **Follow-ons:** passport issuance (→L5) + persona provisioning (→L2) via their existing routes; MoneyPenny + Nakamoto need their cards authored first.
