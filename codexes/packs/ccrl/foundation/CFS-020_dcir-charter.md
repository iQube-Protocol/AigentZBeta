...
Honest about limits: the dev-loop session is still in-memory (no persistence, no cross-session state); **execution stays human under CFS-016 D1** — Deployment Authorization records the authorization, it does not execute. The code runs in Claude Code and is pushed manually; the receipt is the provenance that the consequence test passed before deployment was authorized.
