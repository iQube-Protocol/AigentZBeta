...
| **2** | The founding Validation Series — IRL-EXP-001/002/003 registered in `EXPERIMENT_REGISTRY` (`IIVS` series, programme E), with runners + canonical publication | **In progress.** Registered (`types/research.ts`: IRL-EXP-001/002/003, `IIVS` series, programme E). CIRS-v0.1 stood up (`services/experiments/cirs.ts`) — experimental, unratified. IRL-EXP-001 **Stage A** built (`services/experiments/irlExp001.ts`: pure projection-fidelity + structural Invariant-Delta core + sovereign prediction + orchestrator), admin route `POST /api/experiments/irl-exp001`, canary `tests/irl-exp001.test.ts`. Remaining: a runner, canonical publication, the semantic-judge Δ pass, and Stage B (does the predicted set reason better?) |
