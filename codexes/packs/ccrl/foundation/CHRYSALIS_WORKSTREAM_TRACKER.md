...
| 54 | **CFS-028 Capability Graph & Production Routing — DRAFTED 2026-07-12, awaiting ratification.** Producer (harness/model/delegate) × capability (AR profile) edges with seeded fitness, receipt-learned later, cost stubbed. Unifies D2 execution with production routing: the executor is a Producer whose capability is `deployment-execution` — operator direction recorded: START with the Claude Code session as executor, architect for the server-side executor + other execution agents as Producers earning the climb via predetermined standing (the sovereignty ladder = the graph's standing axis). Law XI: routing recommends, operator selects. v1 increment set in §4. | F | Ratify CFS-028 §4 |
