      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-gray-50 rounded-lg">
            ...
            <p className="text-xs text-gray-600 mt-1 line-clamp-2">{shareText}</p>
            {personaId && (
            ...
            {socialPlatforms.map((platform) => {
              ...
              return (
                <Button
                  key={platform.name}
                  variant="outline"
                  className={`justify-start gap-2 ${platform.color} text-white border-none`}
                  onClick={() => handleSocialShare(platform.name, platform.getUrl())}
                >
          ...
          <div className="p-2 bg-gray-100 rounded text-xs">
            <p className="font-medium mb-1">Deep Link:</p>
            <p className="text-gray-600 break-all">{deepLink}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
...
function generateArticleDeepLink(article: any, personaId?: string): string {
