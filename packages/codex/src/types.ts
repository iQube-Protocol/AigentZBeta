...
export interface CodexAccessRules {
  ...
  rewards?: {
    earnQc?: number;
    earnQct?: number;
    badges?: string[];  // Badge IDs from CRM
  };
  /** Access gates and requirements */
  gates?: {
    minReputation?: number;          // RQH reputation threshold
    requiredPersonaTags?: string[];  // e.g. ["investor", "builder"]
    requiredQuests?: string[];       // CRM quest IDs (PoKW tasks)
    requireSubscription?: boolean;   // Subscription tier requirement
  };
}
