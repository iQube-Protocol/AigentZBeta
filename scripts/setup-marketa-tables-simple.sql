-- =============================================================================
-- Marketa Database Setup Script (Simple Version)
-- This version skips triggers to avoid conflicts
-- =============================================================================
...
DROP POLICY IF EXISTS "Enable all access for development" ON marketa.marketa_multi_tenant_campaigns;
CREATE POLICY "Enable all access for development" ON marketa.marketa_multi_tenant_campaigns FOR ALL USING (true);

DROP POLICY IF EXISTS "Enable all access for development" ON marketa.marketa_tenant_campaign_configs;
CREATE POLICY "Enable all access for development" ON marketa.marketa_tenant_campaign_configs FOR ALL USING (true);

DROP POLICY IF EXISTS "Enable all access for development" ON marketa.marketa_partner_rewards;
CREATE POLICY "Enable all access for development" ON marketa.marketa_partner_rewards FOR ALL USING (true);

DROP POLICY IF EXISTS "Enable all access for development" ON marketa.marketa_delivery_logs;
...
BEGIN
    ...
    RAISE NOTICE '⚠️  Triggers skipped to avoid conflicts (updated_at will be set by application)';
