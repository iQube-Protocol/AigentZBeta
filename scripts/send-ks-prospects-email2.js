#!/usr/bin/env node
/**
 * KS Prospects — Send Email 2: Interest / Why it exists
 *
 * Targets contacts in ks_backers_staging who received Email 1
 * (engagement_status = 'sent') and are still active (suppression_status = 'active').
 * Also picks up anyone who opened Email 1 but hasn't clicked (engagement_status = 'opened')
 * since that group receives the normal sequence through Email 3.
 *
 * Segment C (clicked, not converted) will be handled by a separate Email 4 script.
 * Segment D (converted/backed) are already excluded via suppression_status.
 * Segment E (bounced/unsubscribed) are excluded via suppression_status = 'suppressed'.
 *
 * Embeds CustomID = "stg_<id>|2" so the Mailjet webhook can attribute
 * open/click/bounce events back to ks_backers_staging without an email lookup.
 *
 * Usage:
 *   node scripts/send-ks-prospects-email2.js [--dry-run]
 *
 * Required env:
...
const TEMPLATE_ID   = parseInt(process.env.MAILJET_TEMPLATE_KS_PROSPECTS_02 || "0", 10);
const FROM_EMAIL    = process.env.MAILJET_FROM_EMAIL || "info@metame.com";
const FROM_NAME     = "Dele Atanda";

// Contacts eligible for Email 2 are those who got Email 1 (sent) OR opened it
// but haven't clicked through. Segment C (clicked) will receive Email 4 directly.
const ELIGIBLE_STATUSES = ["sent", "opened"];
...
if (!TEMPLATE_ID) {
  console.error("Missing MAILJET_TEMPLATE_KS_PROSPECTS_02 — run mailjet_create_ks_prospects_templates.py first");
  process.exit(1);
}
...
async function loadContacts() {
  console.log(`Loading contacts (engagement_status in [${ELIGIBLE_STATUSES.join(", ")}])…`);
  const contacts = [];
  let offset = 0;
  const limit = 1000;
  const statusFilter = `(${ELIGIBLE_STATUSES.map(s => `"${s}"`).join(",")})`;

  while (true) {
    const rows = await supabaseRequest(
      `/ks_backers_staging?select=id,first_name,email&suppression_status=eq.active&engagement_status=in.${statusFilter}&limit=${limit}&offset=${offset}`
    );
...
async function markSent(ids) {
  ...
  await supabaseRequest(
    ...
    "PATCH",
    { engagement_status: "email_2_sent", last_sent_at: new Date().toISOString() }
  );
}
...
async function main() {
  console.log(`KS Prospects — Send Email 2: Interest / Why it exists${DRY_RUN ? " [DRY RUN]" : ""}`);
  ...
  for (let i = 0; i < contacts.length; i += BATCH_SIZE) {
    ...
    const messages = batch.map((c) => ({
      ...
      // stg_ prefix tells the webhook handler to look up in ks_backers_staging
      CustomID:         `stg_${c.id}|2`,
      Variables: {
  ...
  console.log("engagement_status updated to 'email_2_sent' for delivered contacts.");
}
