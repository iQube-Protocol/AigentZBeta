...
async function deployQCTRunes() {
  ...
  const terms = new Terms(
    BigInt(QCT_CONFIG.amountPerMint * 10**QCT_CONFIG.decimals), // amount per mint (in smallest units)
    BigInt(QCT_CONFIG.cap), // cap (max mints)
  ...
  const etching = new Etching(
    ...
    some(BigInt(QCT_CONFIG.premine * 10**QCT_CONFIG.decimals)), // premine amount
    some(rune), // rune
    none(), // spacers
