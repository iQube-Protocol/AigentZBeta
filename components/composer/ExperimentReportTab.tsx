...
/**
 * Objective, data-driven EXP-006 → EXP-006A narrative. Reads the latest published
 * aggregates and states them honestly: the exact-set F1 is a LOWER BOUND distorted
 * by abstraction mismatch, not a competence score. Frames why EXP-006 is frozen
 * and EXP-006A supersedes it, and the engine roles EXP-006 clarified. Generated
 * live from the canonical record — the numbers are whatever was published.
 */
function exp006Section(rows: PublishedResult[]): string {
  const pctf = (v: number | null) => (v === null ? "—" : `${Math.round(v * 100)}%`);
  const head = "## 6. EXP-006 → EXP-006A — Intent → Invariant Projection Fidelity (Invariant Intelligence Series)";
  const framing = `
**Aim.** Move from *outcome* evaluation to *mechanism* evaluation: can the invariant projection engine identify the governing invariant set for an intent **independently of a human-authored reference** — i.e. did it select the right constitutional substrate before reasoning began?
...
**Method (Stage A).** For each intent in an independently-generated reference set (CIRS), the sovereign, invariant-aware router predicts the projection; it is scored against the *blind* reference; every disagreement is classified as an Invariant Delta. The reference is generated by a different reasoning stage/provider than the predictor, so the deltas are real cross-model disagreements, not self-agreement.`;
  ...
  if (rows.length === 0) {
    return `${head}\n${framing}\n\n**Status.** Run complete; canonical publication pending. Superseded as the *fidelity metric* by **EXP-006A** (topology / abstraction-aware, below).\n`;
  }
...
function buildReport(results: PublishedResult[], now: Date): string {
  ...
  // EXP-006 has its own authored-objective section (§6); exclude it from the bare
  // auto-list so it isn't duplicated as a table-only entry.
  const NARRATED_IDS = new Set(["EXP-001", "EXP-002", "EXP-003", "EXP-006"]);
  const extraIds = Array.from(new Set(results.map((r) => r.experiment)))
    .filter((id) => !NARRATED_IDS.has(id))
    .sort();
  const extraSection =
    extraIds.length === 0
      ? ""
      : `
## 10. Additional experiments (auto-included from the canonical record)
...
${exp006Section(byExp("EXP-006"))}
## 7. Cross-cutting conclusions (current)
...
## 9. What we're inviting partners to do
