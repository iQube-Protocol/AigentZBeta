...
export class OpenClawWorker {
  ...
  private async tryRunMoltComicsPublish(
    ...
    try {
      ...
      const reuseContinuableChains = process.env.MOLTCOMICS_REUSE_CONTINUABLE === "true";

      let chainId: string | undefined;
      let chainUrl: string | undefined;

      if (hasTool("moltcomics.chains.create")) {
        ...
        if (chainId && this.isSyntheticMoltChainId(chainId)) {
          payload.molt_warning =
            "MoltComics chain create appears synthetic/stubbed (likely API rate limit). Suppressing link publish.";
          chainId = undefined;
          chainUrl = undefined;
        }
      }

      if (!chainId && reuseContinuableChains && hasTool("moltcomics.chains.continuable")) {
      ...
      if (chainId && hasTool("moltcomics.panels.submit")) {
        ...
        this.pushOutboundLink(links, "View Comic Panel", panelUrl, chainId);
      }

      if (chainUrl) {
        this.pushOutboundLink(links, "Open MoltComics Chain", chainUrl, chainId);
      } else if (chainId) {
        this.pushOutboundLink(links, "Open MoltComics Chain", `https://moltcomics.com/chains/${chainId}`, chainId);
      }
  ...
  private isSyntheticMoltChainId(chainId: string): boolean {
    return chainId.startsWith("chain_");
  }

  private isSyntheticMoltUrl(url: string): boolean {
    return /\/chains\/chain_[A-Za-z0-9_-]*/.test(url) || /\/panel\/panel_[A-Za-z0-9_-]*/.test(url);
  }

  private pushOutboundLink(
    links: OutboundLink[],
    label: string,
    ...
    if ((chainId && this.isSyntheticMoltChainId(chainId)) || this.isSyntheticMoltUrl(url)) {
      return;
    }
