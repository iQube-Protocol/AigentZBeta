-- 20260930002800_agent_delegate_stood_up_receipt_type.sql
--
-- Adds `agent_delegate_stood_up` — Aletheon Homecoming Stage 1 preflight
-- (operator-directed, 2026-08-15).
--
-- A Homecoming delegate's agent_root_identity and/or agent_persona was newly
-- created via POST /api/homecoming/agent/stand-up (the generic genesis
-- pipeline, services/homecoming/agentHomecoming.ts::standUpDelegate). Records
-- the completed L0->L2 mechanical transition; emitted best-effort AFTER the
-- state writes succeed and never gates them. Fired only when at least one of
-- the two rows was freshly created in this call. Carries agent-scoped
-- identifiers only (agent_root_id, agent_id, agent_card_slug, agent_class) --
-- never a sponsor persona/passport id.
--
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- Aletheon Homecoming Stage 1 preflight (operator-directed, 2026-08-15) — see this migration's own header.
