-- 20260930002900_agent_delegation_anchor_repaired_receipt_type.sql
--
-- Adds `agent_delegation_anchor_repaired` — Chrysalis Homecoming (CFS-023)
-- constitutional anchoring repair (operator-directed, 2026-08-15).
--
-- A legacy polity-bound delegate whose mechanical stand-up (agent_root_identity
-- + agent_persona) already completed, but whose sponsor's identity never
-- resolved through provisionAgentPersona.ts's root_did string match (that
-- file's own comment names this exact condition and flags it "for later
-- backfill"), had delegation_user_root_id / delegation_persona_id filled via
-- services/agents/repairDelegationAnchor.ts. Forward-looking only: describes
-- the REPAIR act, dated to when it happened; never a fabricated historical
-- genesis receipt for the original stand-up.
--
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- Chrysalis Homecoming constitutional anchoring repair (operator-directed, 2026-08-15) — see this migration's own header.
