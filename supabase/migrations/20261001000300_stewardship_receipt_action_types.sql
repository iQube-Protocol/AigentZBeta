-- 20261001000300_stewardship_receipt_action_types.sql
--
-- Rebuilds activity_receipts_action_type_check WHOLESALE with the complete
-- current ActivityActionType list plus EIGHT new entries for IRL Stewardship
-- — Access Maintenance + Research Persona + Capability model:
--
--   'access_grant_amended'            — scope/role/expiry changed on an
--                                        existing active grant (item 1)
--   'access_grant_suspended'          — grant put on a reversible hold
--   'access_grant_reinstated'         — a suspended grant reactivated
--   'access_grant_revoked'            — grant permanently revoked (distinct
--                                        from access_invitations' own revoke)
--   'access_grant_capability_granted' — a capability row added/renewed at a
--                                        resource scope (item 15)
--   'access_grant_capability_revoked' — a capability row revoked (immediate
--                                        effect, item 15)
--   'research_persona_proposed'       — a steward proposed a research
--                                        persona/handle at invitation time
--   'research_persona_updated'        — the persona/handle was created,
--                                        confirmed, or later edited (item 3)
--
-- Separate types rather than one generic 'access_changed', for the same
-- reason `standing_accrued`/`standing_corrected` and the nine venture-trading
-- types stay separate (see their own migrations' comments): these are
-- independent facts with independent consequences, and a single shared type
-- would make suspension indistinguishable from revocation in the receipt
-- trail — exactly the ambiguity the 'suspended' status column was added to
-- avoid.
--
-- Carried forward VERBATIM from
-- 20260930300000_didqube_container_bound_receipt_action_type.sql plus
-- exactly these 8 new entries, per
-- tests/activity-receipts-action-type-parity.test.ts's governing rule: every
-- migration extending the TS union must rebuild this constraint wholesale.
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- New this migration (2026-10-01) — IRL Stewardship, see header.
