...
--
-- Mirrors scripts/create-store-skus-table.sql which was previously a manual
-- one-off seed. Promoting it to a real migration so fresh environments and
-- redeploys auto-seed the catalog. Without these rows, services/rewards/
-- assetOwnership.getOwnedSkus returns empty arrays, no SKU expansion happens
-- in getOwnedAssetIds, and bundle purchases never surface as accessible
-- content in the codex (manifesting as "user paid but has no access").
--
-- Idempotent: ON CONFLICT updates the row in place. Safe to re-run.
-- =============================================================================
...
CREATE TABLE IF NOT EXISTS store_skus (
  ...
  -- Optional per-SKU episode-number scoping (NULL = unscoped / global)
...
INSERT INTO store_skus (sku_id, name, description,
  ...
  -- Public episode bundles (not investor) — episode_numbers scoping limits
  -- expansion to the bundle's listed episode range. Database episode numbers
  -- are 1-indexed (DB) vs 0-indexed (pricing convention) — these are DB.
  ('bundle-0-2',    'Episodes 0-2',     '3-episode public bundle (Eps 0-2)',
    true, false, false, false, false),
  ('bundle-3-7',    'Episodes 3-7',     '5-episode public bundle (Eps 3-7)',
    true, false, false, false, false),
  ('bundle-8-12',   'Episodes 8-12',    '5-episode public bundle (Eps 8-12)',
    true, false, false, false, false),
  ('bundle-full',   'Full Season 0-12', 'Full season public bundle (13 episodes)',
    true, false, false, false, false)
