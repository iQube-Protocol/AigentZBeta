-- 20260930001200_wallet_repair_receipt_types.sql
--
-- Wallet repair (operator ruling 2026-08-02, wallet-binding trace #121) — five
-- INDEPENDENT evidence types for the single-persona principal-wallet repair.
--
-- Separate types rather than one `wallet_repaired` because they are separate
-- facts with separate consequences. Superseding a keyless placeholder and
-- provisioning a principal wallet can each happen without the other. And
-- proving control of a LINKED EXTERNAL wallet must never read as proving
-- control of the principal one — a single receipt type would make those two
-- indistinguishable in the ledger, which is the same conflation the mint route
-- made in the persona row.
--
-- See services/wallet/linkedExternalWallet.ts (WALLET_REPAIR_RECEIPT_TYPES).
