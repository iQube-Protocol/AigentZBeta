-- 20260930003100_implementation_execution_observed_receipt_type.sql
--
-- Adds `implementation_execution_observed` — DevOn Phase F bounded-execution
-- repair (operator-directed, 2026-08-16).
--
-- The observation ledger for one implementation-actor dispatch (today:
-- anthropics/claude-code-action@v1 via .github/workflows/claude-implement.yml):
-- which provider/model actually ran, turns, wall-clock duration, tokens,
-- observed cost, permission-denial count, and the resulting execution state
-- (proceeding/awaiting-escalation/complete). Extracted from the actor's own
-- terminal result JSON via services/constitutional/executionTelemetry.ts.
-- This is NOT the live governor — services/constitutional/executionBudget.ts's
-- evaluateBudget() is — this receipt is durable observation only, closing the
-- forensic audit's finding that the only prior visibility into a dispatch's
-- cost was a raw job log an operator had to find and read manually.
--
