-- 20260930000900_register_ceremony_receipt_types.sql
--
-- Wallet Signing Topology (operator ruling 2026-08-01) — five INDEPENDENT
-- Register-stage evidence types, one per ceremony step (principal mandate
-- signed -> agent-wallet approval signs the tx -> broadcast submitted ->
-- Horizen reread confirms -> binding recorded), so Register can only reach
-- COMPLETE once every step of the wallet-mediated ceremony has actually
-- happened. See services/horizen/registerCeremony.ts and
-- codexes/packs/agentiq/updates/2026-08-01_wallet-signing-topology-ruling-and-phase-1.md.
-- horizen_agent_registered remains as-is (pre-ceremony completion evidence,
-- never replaced).
