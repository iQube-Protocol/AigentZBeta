-- 20261001000700_vela_underwriting_projection_receipt_type.sql
--
...
--
--   'vela_underwriting_projection_completed' — Use Case Zero's first
--   underwriting vertical slice (2026-09-13,
--   services/vela/velaUnderwritingProjection.ts). Records a completed Vela
--   multi-party confidential-projection run whose disclosed coarse verdict
--   was fed into a pluggable, deterministic underwriting quote provider
--   (services/financialServices/providers/underwriting/). Carries ONLY
--   namespace refs, scope binding, the coarse verdict, and the resulting
--   SIMULATED quote's own fields — never a party's raw financial inputs.
--   DVN-anchorable (services/dvn/activityReceiptDvnPipeline.ts).
--
-- Carried forward VERBATIM from
-- 20261001000600_exchange_access_restored_correction_receipt_type.sql plus
-- exactly this one new entry, per
