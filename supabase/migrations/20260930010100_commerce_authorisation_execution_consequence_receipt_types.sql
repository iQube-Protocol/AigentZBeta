-- 20260930010100_commerce_authorisation_execution_consequence_receipt_types.sql
--
-- Adds the six VELA-001 Constitutional Commerce receipt types (operator-
-- directed, 2026-08-22, downstream of Slice 2F's Gate 2 work):
--
--   commerce_action_authorised    -- ActionAuthorisation.status === 'AUTHORISED'
--   commerce_action_refused       -- ActionAuthorisation.status === 'REFUSED'
--   commerce_action_unresolved    -- ActionAuthorisation.status === 'UNRESOLVED'
--   commerce_execution_bound      -- bindExecution() bound a CommerceExecution
--   commerce_execution_refused    -- bindExecution() refused (not AUTHORISED / expired)
--   commerce_consequence_recorded -- recordObservedConsequence() wrote an ObservedConsequence
--
-- These are FINANCIAL-DOMAIN outcomes, one level downstream of the
-- GOVERNANCE-layer capability_invocation_* types already in this constraint —
-- deriveActionAuthorisation()/bindExecution()/recordObservedConsequence() in
-- services/constitutionalCommerce/ produce them, never invokeCapability()
-- itself. `commerce_action_unresolved` is deliberately its own type rather
-- than folded into `commerce_action_refused`, preserving the same
-- UNACCEPTABLE-vs-UNRESOLVED distinction carried through
-- ConsequenceProjection and ActionAuthorisation: a fail-closed "nothing was
-- established" outcome must stay auditably distinguishable from a "something
-- concrete blocked it" outcome.
--
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- Legacy Passport/personhood linkage reconciliation (operator-directed, 2026-08-15).
    ...
    -- Homecoming Phase II WP-B (operator-directed, 2026-08-16).
    'implementation_execution_returned',
    -- VELA-001 Constitutional Commerce — Authorisation/Execution/Consequence
    -- planes (operator-directed, 2026-08-22) — see this migration's own header.
