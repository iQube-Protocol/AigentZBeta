-- 20260930003300_implementation_execution_returned_receipt_type.sql
--
-- Adds `implementation_execution_returned` — Homecoming Phase II WP-B
-- (operator-directed, 2026-08-16).
--
-- The manual/external-actor counterpart to `implementation_execution_observed`:
-- a human-reviewed, qualitative Execution Return (branch, commits, PR, files
-- changed, validation results, deviations from the pack, failures/escalations,
-- discoveries, consequence observations) submitted via
-- services/constitutional/executionReturn.ts and app/api/constitutional/
-- execution-return/route.ts, accepted only when its packId matches an
-- existing implementation_pack_generated receipt's action_input->>'pack_id'.
-- Never authorizes deployment by itself and never writes
-- `deployment_authorized` — it is evidence a stage-transition gate later
-- reads, not a new authority mechanism (the three-axis model from WP-A is
-- unaffected by this receipt type).
--
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- DevOn Phase F bounded-execution repair (operator-directed, 2026-08-16) — see this migration's own header.
    'implementation_execution_observed',
    -- Homecoming Phase II WP-B (operator-directed, 2026-08-16) — see this migration's own header.
