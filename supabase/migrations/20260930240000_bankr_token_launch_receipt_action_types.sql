-- 20260930240000_bankr_token_launch_receipt_action_types.sql
--
-- Factor + Aegis Bankr PRD, Phase 4/7 — rebuilds activity_receipts_action_type_check
-- WHOLESALE with the complete current ActivityActionType list plus the 8 new
-- Bankr/token-launch entries (services/receipts/activityReceiptService.ts).
-- Carried forward VERBATIM from 20260930190000_factor_aegis_constitution_reconciled.sql
-- plus exactly the 8 new entries — per tests/activity-receipts-action-type-parity.test.ts's
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- Factor + Aegis Bankr PRD, Phase 4/7 (2026-09-05) — new this migration.
