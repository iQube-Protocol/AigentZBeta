-- 20261001001000_vela_underwriting_composition_gate_receipt_types.sql
--
-- Rebuilds activity_receipts_action_type_check WHOLESALE with the complete
-- current ActivityActionType list plus TWO new entries:
--
--   'vela_underwriting_disclosure_authorized' — Use Case Zero build-order
--   item 9 (2026-09-13, services/vela/velaUnderwritingDisclosureAuthorization.ts).
--   A Tier-1 QubeTalk/iQube act: a receiptable authorization of an EXACT
...
--
--   'vela_underwriting_envelope_frozen' — Use Case Zero build-order item 9
--   (2026-09-13, services/vela/velaUnderwritingCompositionGate.ts). The
--   MoneyPenny composition-gate's final causal receipt: the Factor
--   selection, Aegis admission, and disclosure authorization evidence refs
--   frozen into one envelope, then submitted through the EXISTING,
--   unmodified runVelaUnderwritingProjection. Binds all four evidence
--   classes the operator's own ruling required (Factor selection, Aegis
--   admission, disclosure authorization, Vela execution). Carries only
--   opaque refs, the coarse verdict, and the SIMULATED quote's own
--   providerMode — never a party's raw financial inputs.
--
-- Both DVN-anchorable (services/dvn/activityReceiptDvnPipeline.ts).
--
-- Carried forward VERBATIM from
-- 20261001000900_vela_underwriting_admission_evidence_receipt_type.sql plus
-- exactly these two new entries, per
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- New this migration (2026-09-13) — see header.
