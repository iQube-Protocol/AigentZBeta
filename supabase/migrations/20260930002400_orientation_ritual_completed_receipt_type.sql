-- 20260930002400_orientation_ritual_completed_receipt_type.sql
--
-- Adds `orientation_ritual_completed` — the receipt for the new Orient
-- stage's operator acknowledgment act (Threshold Journey — Orient stage +
-- Consequence Fork, operator spec, 2026-08-09).
--
-- Orient sits between Claim and Passport on the Horizen constitutional
-- admission spine (services/journey/horizenMoneyPennyJourney.ts). It answers
...
-- originates?" — resolved from state (services/journey/orientationContext.ts),
-- never from agent name. This receipt is issued ONLY on the operator's
-- explicit acknowledgment act, never merely for viewing the stage.
--
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- Threshold Journey — Orient stage (operator spec, 2026-08-09) — see
