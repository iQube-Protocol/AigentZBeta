-- 20260930002600_agent_registry_activated_receipt_type.sql
--
-- Adds `agent_registry_activated` — Constitutional State Model Correction
-- (operator-ratified, 2026-08-11).
--
-- The Activate stage's own receipt: an agent became a constitutionally
-- active, accountable participant in the iQube Registry. Derived from, and
-- ONLY from, iQubeRegistryPresent ∧ sponsorBindingEstablished ∧
-- agentPassportIssued (services/journey/agentRegistryActivation.ts) — never
-- from Delegate, Operate, or `capability_registered` (Factory/Ingest
-- evidence, which stays technical and is never conflated with this
-- constitutional fact). Awards no Standing.
--
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- Constitutional State Model Correction (operator-ratified, 2026-08-11) — see this migration's own header.
