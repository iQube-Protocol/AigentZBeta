-- 20261001000900_vela_underwriting_admission_evidence_receipt_type.sql
--
-- Rebuilds activity_receipts_action_type_check WHOLESALE with the complete
-- current ActivityActionType list plus ONE new entry:
--
--   'vela_underwriting_admission_evidence_composed' — Use Case Zero
--   build-order item 8 (2026-09-13,
--   services/vela/velaUnderwritingAdmissionEvidence.ts). Reads Factor's
--   selection artifact (item 7) and Aegis's EXISTING ('agent'-subject-type)
--   assessment machinery to produce a deterministic ADMITTED/REFUSED/
--   UNRESOLVED admission-evidence artifact bound to selectionRef +
--   requestRef + candidate + assessment version. Distinct from Aegis's own
--   generic aegis_assessment_requested/aegis_assessment_ratified receipt
--   types (unchanged, not duplicated) — this event is the underwriting
--   flow's own downstream consumption of that verdict, never the
--   assessment itself. DVN-anchorable
--   (services/dvn/activityReceiptDvnPipeline.ts). Contains no raw party
--   financial data.
--
-- Carried forward VERBATIM from
-- 20261001000800_factor_selection_proposed_receipt_type.sql plus exactly
-- this one new entry, per
