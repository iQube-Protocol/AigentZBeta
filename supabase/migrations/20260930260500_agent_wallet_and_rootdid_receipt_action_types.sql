-- 20260930260500_agent_wallet_and_rootdid_receipt_action_types.sql
--
-- Rebuilds activity_receipts_action_type_check WHOLESALE with the complete
-- current ActivityActionType list plus TWO entries missing their own
-- migration: 'agent_purpose_wallet_provisioned' (Use Case Zero orchestrator
-- wallet-provisioning receipts, services/factor/useCaseZeroOrchestrator.ts)
-- and 'agent_root_identity_sponsored' (Use Case Zero RootDID-minting
-- primitive, same file). Both were added to
-- services/receipts/activityReceiptService.ts's ActivityActionType union
-- (and, for the latter, services/dvn/activityReceiptDvnPipeline.ts's
-- ANCHORABLE_ACTION_TYPES) without a matching constraint migration —
-- confirmed missing by tests/activity-receipts-action-type-parity.test.ts,
-- which failed on exactly these two names before this migration. Carried
-- forward VERBATIM from 20260930250000_confidential_projection_receipt_action_type.sql
-- plus exactly the 2 new entries, per that test's own governing rule: every
-- migration extending the TS union must rebuild this constraint wholesale,
-- never just append without the full list.
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- New this migration (2026-09-06) — see header.
