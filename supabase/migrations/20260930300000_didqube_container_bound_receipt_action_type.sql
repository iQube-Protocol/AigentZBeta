-- 20260930300000_didqube_container_bound_receipt_action_type.sql
--
...
-- 'agent_didqube_container_bound' (DiDQube Phase 4 item 1, 2026-09-07 —
-- Factor's Use Case Zero orchestrator's didqubeContainer step,
-- services/factor/useCaseZeroOrchestrator.ts, binding an existing
-- agent_root_identity into its DiDQube constitutional container via
-- services/identity/didQubeResolver.ts::ensureAgentDiDQubeBinding). Local
-- only — deliberately NOT added to services/dvn/activityReceiptDvnPipeline.ts's
-- ANCHORABLE_ACTION_TYPES; the DiDQube DVN consumer/payload migration stays
-- behind its own required standalone approval (execution plan Phase 4 item 6).
-- Carried forward VERBATIM from
-- 20260930260500_agent_wallet_and_rootdid_receipt_action_types.sql plus
-- exactly the 1 new entry, per tests/activity-receipts-action-type-parity.test.ts's
-- governing rule: every migration extending the TS union must rebuild this
-- constraint wholesale, never just append without the full list.
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- New this migration (2026-09-07) — see header.
