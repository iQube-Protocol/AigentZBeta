-- 20260930002500_pnl_service_registered_receipt_type.sql
--
-- Adds `pnl_service_registered` — Horizen Pilot Closure, part C (2026-08-09).
--
-- A THIRD, genuinely distinct P&L fact, alongside the two that already
-- exist:
...
--     onboarding (POST /v1/register) has SUCCEEDED for this agent — an
--     internal PnL agentId (UUID) now exists. Says nothing about whether
--     any proof evidence exists yet.
...
--
-- See services/horizen/pnlOnboardingClient.ts (the dedicated mutating
-- boundary this receipt type belongs to — pnlServiceVerification.ts stays
-- read-only).
--
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- Horizen Pilot Closure, part C (2026-08-09) — see this migration's own header.
