-- 20260930090000_activity_receipts_locker_action_types.sql
--
-- Extends activity_receipts.action_type with Locker/RoomQube/SharePack
-- event types (Locker/RoomQube/Share Packs spec §18/§14.4). This is the
-- ONE permitted unilateral change under CLAUDE.md's DVN Pipeline
-- Protection section: "Adding a new action type to
-- ANCHORABLE_ACTION_TYPES... does not modify the submission mechanism,
-- state machine, or canister interaction." The corresponding code-side
-- union lives in services/receipts/activityReceiptService.ts
-- (ActivityActionType) — no new receipts table.
--
-- Regenerated against THIS branch's real, current action-type list (159
-- entries through 20260930070000_qubetalk_publication_execution.sql) — the
-- originating worktree's own version of this migration was built against
-- an isolated, unrelated-history checkout whose action-type list had only
-- 9 entries; applying it as-is here would have wiped ~150 real, live
-- action types via the wholesale CHECK-constraint rebuild this repo's own
-- pattern requires (see tests/activity-receipts-action-type-parity.test.ts).
-- Every entry below through 'qubetalk_publication_projection_published' is
-- carried forward verbatim; only the nine 'locker_*' types at the end are
-- new.
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- Locker / RoomQube / Share Pack (2026-08-25, spec §18).
