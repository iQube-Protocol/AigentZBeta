-- 20261001000500_exchange_artifact_operator_text_receipt_type.sql
--
...
--
--   'exchange_artifact_operator_text_attached' — an operator attached a
--   verified plaintext fallback to an already-deposited exchange artifact
--   when automated content extraction could not reach the underlying bytes
--   (setArtifactOperatorProvidedText, services/research/reciprocalExchange.ts).
--   Never touches the artifact's fingerprint fields — a distinct fact from
--   'exchange_artifact_deposited'/'exchange_artifact_replaced', so it gets
--   its own type rather than overloading either.
--
-- Carried forward VERBATIM from
-- 20261001000300_stewardship_receipt_action_types.sql plus exactly this one
-- new entry, per tests/activity-receipts-action-type-parity.test.ts's
-- governing rule: every migration extending the TS union must rebuild this
-- constraint wholesale.
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    -- New this migration (2026-09-12) — see header.
