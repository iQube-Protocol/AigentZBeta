-- 20260930002000_pulse_reconciliation_receipt_types.sql
--
...
-- authority with receipted constitutional state"). Wholesale CHECK-constraint
-- rebuild per the drift-incident regression guard
...
--
-- Adds three action types (see services/horizen/authorizationClient.ts's
-- writeConfirmedPulseActivation / reconcilePulseConstitutionalState):
--   pulse_enrollment_verified               -- fine-grained transition, fires
--                                               whenever a Pulse authorization
--                                               confirms (alongside the
--                                               existing horizen_pulse_authorized)
--   pulse_commitment_verified               -- fires only when the SAME
--                                               confirmation also reports
--                                               pulseCommitmentRecorded: true
--   horizen_reconciliation_discrepancy_recorded -- fires when a LATER
--                                               reconciliation read disagrees
--                                               with already-receipted evidence;
--                                               never rewrites the receipted
--                                               transition it compares against
