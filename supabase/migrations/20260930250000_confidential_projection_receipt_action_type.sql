-- 20260930250000_confidential_projection_receipt_action_type.sql
--
-- Factor + Aegis PRD tranche (2026-09-05) — rebuilds activity_receipts_action_type_check
-- WHOLESALE with the complete current ActivityActionType list plus the one new
-- entry (services/receipts/activityReceiptService.ts) covering Factor's Vela
-- confidential-projection workloads (services/factor/factorConfidentialWorkload.ts).
-- Carried forward VERBATIM from 20260930240000_bankr_token_launch_receipt_action_types.sql
-- plus exactly the 1 new entry — per tests/activity-receipts-action-type-parity.test.ts's
-- own governing rule: every migration extending the TS union must rebuild this
...
ALTER TABLE public.activity_receipts
  ...
  CHECK (action_type IN (
    ...
    'token_fees_claimed',
    -- Factor + Aegis PRD tranche (2026-09-05) — new this migration.
