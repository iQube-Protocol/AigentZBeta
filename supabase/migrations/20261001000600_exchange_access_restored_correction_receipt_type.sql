-- 20261001000600_exchange_access_restored_correction_receipt_type.sql
--
...
--
--   'exchange_access_restored_operator_correction' — a mistaken post-exchange
--   revocation reversed by hand. The exchange state machine deliberately has
--   no forward transition out of REVOKED_ACCESS_POST_EXCHANGE (see
--   types/reciprocalExchange.ts's own comment on exception-state terminality);
--   this receipt type records the operator-governed correction act itself,
--   never a first-class "restore" product transition.
--
-- Carried forward VERBATIM from
-- 20261001000500_exchange_artifact_operator_text_receipt_type.sql plus
