-- 20260930002200_pnl_service_verified_receipt_type.sql
--
-- Adds `pnl_service_verified` — the receipt for an INDEPENDENT, read-only
-- Horizen Verifiable-PnL correlation, issued only when
-- discoverAndReceiptPnlServiceEvidence (services/horizen/pnlServiceVerification.ts)
-- finds a genuine, attributable PnL record for the exact agent/token/chain
-- (operator directive, 2026-08-08).
--
-- Deliberately additive alongside two EXISTING, materially different
-- receipts, never replacing either:
--   - `horizen_pnl_transparency_enabled` — issued UNCONDITIONALLY alongside
--     Pulse confirmation; represents "disclosure scope was authorized",
--     not "the PnL service was independently verified".
--   - `partner_agent_evidence_recorded` — the receipt for an IDENTITY-BINDING
--     claim, a different constitutional question (who may exercise Standing
--     for this agent) requiring a resolved attribution binding this new
