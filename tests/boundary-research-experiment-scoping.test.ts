...
describe('research-active reuses PartnerProgrammesTab — never a forked Boundary Research engine', () => {
  ...
  it('keeps "Explore IRL OS" as the explicit route into the larger body of work, and actually routes to IRL OS (access-boundary correction, 2026-08-26; repointed off the disabled irl-os-workspace 2026-08-27 -- see tests/irl-os-access-boundary.test.ts and docs/security/2026-08-27_irl-os-containment-breach-audit.md)', () => {
    const src = stripComments(readSource('components/journey/BoundaryResearchProgressPanel.tsx'));
    expect(src).toContain('Explore IRL OS');
    expect(src).toContain("buildCodexUrl('irl-os-cartridge', { tab: 'irl-os-welcome'");
